...
class Cron_model extends CI_Model
{
    ...
    private function updateInvoiceStatus($id = false)
    {
    ...
    function sent_email($details = false) {
        if ($details) {
            $table_html = '';
    ...
    private function gen_html($costing = false, $discount = false, $expenses = false, $returns = false, $purchases = false, $sales = false, $warehouse = NULL) {
        ...
        <tr>
            ...
            <td style="text-align:right; border-bottom: 1px solid #EEE;">'.$this->cus->formatMoney($costing->sales).'</td>
        </tr>';
        if ($discount && $discount->order_discount > 0) {
            $html .= '
            <tr>
                <td style="border-bottom: 1px solid #DDD;">'.lang('order_discount').'</td>
                <td style="text-align:right;border-bottom: 1px solid #DDD;">'. $this->cus->formatMoney($discount->order_discount).'</td>
            </tr>';
        }
        $html .= '
        <tr>
            <td style="border-bottom: 1px solid #EEE;">'.lang('products_cost').'</td>
            <td style="text-align:right; border-bottom: 1px solid #EEE;">'.$this->cus->formatMoney($costing->cost).'</td>
        </tr>';
        if ($expenses && $expenses->total > 0) {
            $html .= '
            <tr>
                <td style="border-bottom: 1px solid #DDD;">'.lang('expenses').'</td>
                <td style="text-align:right;border-bottom: 1px solid #DDD;">'. $this->cus->formatMoney($expenses->total).'</td>
            </tr>';
        }
        $html .= '
        <tr>
            ...
            <td style="text-align:right;border-bottom: 1px solid #DDD;">
                <strong>'.$this->cus->formatMoney($costing->sales - $costing->cost - ($discount ? $discount->order_discount : 0) - ($expenses ? $expenses->total : 0)).'</strong>
            </td>
        </tr>';
        if (isset($returns->total)) {
            ...
            <tr>
                ...
                <td style="text-align:right;border-bottom: 2px solid #DDD;"><strong>'.$this->cus->formatMoney($returns->total).'</strong></td>
            </tr>';
        }
        ...
        if ($sales) {
            $html .= '
            <tr>
                <td width="33%" style="border-bottom: 1px solid #DDD;">'.lang('total_sales').': <strong>'.$this->cus->formatMoney($sales->total_amount).'('.$sales->total.')</strong></td>
                <td width="33%" style="border-bottom: 1px solid #DDD;">'.lang('received').': <strong>'.$this->cus->formatMoney($sales->paid).'</strong></td>
                <td width="33%" style="border-bottom: 1px solid #DDD;">'.lang('taxes').': <strong>'.$this->cus->formatMoney($sales->tax).'</strong></td>
            </tr>';
        }
        if ($purchases) {
            $html .= '
            <tr>
                <td width="33%">'.lang('total_purchases').': <strong>'.$this->cus->formatMoney($purchases->total_amount).'('.$purchases->total.')</strong></td>
                <td width="33%">'.lang('paid').': <strong>'.$this->cus->formatMoney($purchases->paid).'</strong></td>
                <td width="33%">'.lang('taxes').': <strong>'.$this->cus->formatMoney($purchases->tax).'</strong></td>
            </tr>';
        }
        $html .= '</table></div>';
        return $html;
    ...
    private function getCosting($date = false, $warehouse_id = NULL)
    {
    ...
    private function getOrderDiscount($sdate = false, $edate = false, $warehouse_id = NULL)
    {
    ...
    private function getExpenses($sdate = false, $edate = false, $warehouse_id = NULL)
    {
    ...
    private function getReturns($sdate = false, $edate = false, $warehouse_id = NULL)
    {
    ...
    private function getTotalPurchases($sdate = false, $edate = false, $warehouse_id = NULL)
    {
    ...
    private function getTotalSales($sdate = false, $edate = false, $warehouse_id = NULL)
    {
        $this->db->select('count(id) as total, sum(COALESCE(grand_total, 0)) as total_amount, SUM(COALESCE(paid, 0)) as paid, SUM(COALESCE(total_tax, 0)) as tax', FALSE)
