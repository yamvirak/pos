...
class Auth_model extends CI_Model
{
    ...
    public function hash_password($password = false, $salt = false, $use_sha1_override = FALSE)
    {
        if (empty($password)) {
            return FALSE;
        }
    ...
    public function hash_password_db($id = false, $password = false, $use_sha1_override = FALSE)
    {
	...
	public function hash_password_cus_db($id = false, $password = false, $use_sha1_override = FALSE)
    {
        if (empty($id) || empty($password)) {
            return FALSE;
        }
    ...
    public function hash_code($password = false)
    {
    ...
    public function deactivate($id = NULL)
    {
        ...
        if ($return) {
			...
			IF($user){
				$userID_Length = strlen($user->username);
				$now = time() - (10);
				$statement = 'SELECT id FROM cus_sessions WHERE timestamp >= ".$now." AND data LIKE \'%s:' . $userID_Length . ':"' . $user->username . '";%\'';
				$sq = $this->db->query($statement);
				
				if ($sq->num_rows() > 0) {
					$ss = $sq->result();
					foreach ($ss as $s) {
						if (!$this->db->delete('cus_sessions', array('id' => $s->id))) {
						}
					}
				}
			}
    ...
    public function clear_forgotten_password_code($code = false)
    {
    ...
    public function reset_password($identity = false, $new = false)
    {
    ...
    public function forgotten_password($identity = false)
    {
        if (empty($identity)) {
    ...
    public function forgotten_password_complete($code = false, $salt = FALSE)
    {
    ...
    public function register($username = false, $password = false, $email = false, $additional_data = array(), $active = FALSE)
    {
	...
	public function clogin($identity = false, $password = false, $remember = FALSE)
    {
        ...
        $this->trigger_events('extra_where');
        $query = $this->db->select('id, username, password, name, active')
        ...
        if ($query->num_rows() === 1) {
            $user = $query->row();
            $password = $this->hash_password_cus_db($user->id, $password);
            if ($password === TRUE) {
                if ($user->active != 1) {
                    $this->trigger_events('post_login_unsuccessful');
				...
				$user->id = 999999 + $user->id;
				$this->session->set_userdata('user_id', $user->id);
                $this->session->set_userdata('identity', true);
    ...
    public function login($identity = false, $password = false, $remember = FALSE)
    {
        $this->trigger_events('pre_login');
        ...
        $query = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login, last_ip_address, avatar, gender, group_id, warehouse_id, biller_id, project_ids, company_id, view_right, edit_right, allow_discount, show_cost, show_price, floor_ids, style_view, cus_style')
    ...
    public function is_max_login_attempts_exceeded($identity = false)
    {
    ...
    function get_attempts_num($identity = false)
    {
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            $ip_address = $this->_prepare_ip($this->input->ip_address());
    ...
    public function is_time_locked_out($identity = false)
    {
    ...
    public function get_last_attempt_time($identity = false)
    {
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            $ip_address = $this->_prepare_ip($this->input->ip_address());
    ...
    public function increase_login_attempts($identity = false)
    {
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            $ip_address = $this->_prepare_ip($this->input->ip_address());
    ...
    public function clear_login_attempts($identity = false, $expire_period = 86400)
    {
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            $ip_address = $this->_prepare_ip($this->input->ip_address());
    ...
    public function where($where = false, $value = NULL)
    {
    ...
    public function like($like = false, $value = NULL, $position = 'both')
    {
    ...
    public function order_by($by = false, $order = 'desc')
    {
    ...
    public function update($id, array $data, $upgs = array())
    {
		...
		IF($data['active'] != 1){
			$userID_Length = strlen($data['username']);
			$now = time() - (10);
			$statement = 'SELECT id FROM cus_sessions WHERE timestamp >= ".$now." AND data LIKE \'%s:' . $userID_Length . ':"' . $data['username'] . '";%\'';
			$sq = $this->db->query($statement);
			if ($sq->num_rows() > 0) {
				$ss = $sq->result();
				foreach ($ss as $s) {
    ...
    public function update_last_login($id = false)
    {
    ...
    public function update_last_login_ip($id = false)
    {
    ...
    public function set_session($user = false)
    {
        ...
        $session_data = array(
			...
			'style_view' => $user->style_view,
			'cus_style' => $user->cus_style,
        );
    ...
    public function login_remembered_user()
    {
        ...
        //get the user
        $this->trigger_events('extra_where');
        $query = $this->db->select($this->identity_column . ', id, username, email, last_login, last_ip_address, avatar, gender, group_id, warehouse_id, biller_id,project_ids, company_id, view_right, allow_discount, edit_right, show_cost, show_price, floor_ids, style_view, cus_style')
    ...
    public function set_hook($event = false, $name = false, $class = false, $method = false, $arguments = false)
    {
    ...
    public function remove_hook($event = false, $name = false)
    {
    ...
    public function remove_hooks($event = false)
    {
    ...
    protected function _call_hook($event = false, $name = false)
    {
    ...
    public function trigger_events($events = false)
    {
    ...
    public function set_message_delimiters($start_delimiter = false, $end_delimiter = false)
    {
    ...
    public function set_error_delimiters($start_delimiter = false, $end_delimiter = false)
    {
    ...
    protected function _filter_data($table = false, $data = false)
    {
        $filtered_data = array();
    ...
    protected function _prepare_ip($ip_address = false)
    {
    ...
    public function updateAvatar($id = false, $avatar = false)
    {
    ...
    public function getBillingAddress($id = false)
    {
    ...
    public function getDeliveryAddress($id = false)
    {
    ...
    public function updateUser($id = false, $admin = false)
    {
	...
	public function getAllGroupPrices()
	{
	...
	public function setSalesmanProductCommission($product_id = false, $salesman_id = false, $commission = false)
    {
		if($product_id && $salesman_id){
			if ($this->getSalesmanCommission($salesman_id, $product_id)) {
				if ($this->db->update('product_commissions', array('commission' => $commission), array('salesman_id' => $salesman_id, 'product_id' => $product_id))) {
					return true;
				}
			} else {
				if ($this->db->insert('product_commissions', array('commission' => $commission, 'salesman_id' => $salesman_id, 'product_id' => $product_id))) {
					return true;
				}
			}
		}
	...
	public function deleteSalesmanProductCommission($product_id = false, $salesman_id = false)
    {
        if ($this->db->delete('product_commissions', array('salesman_id' => $salesman_id, 'product_id' => $product_id))) {
            return TRUE;
        }
        return FALSE;
    }
	
	public function getSalesmanCommission($salesman_id = false, $product_id = false)
    {
        $q = $this->db->get_where('product_commissions', array('salesman_id' => $salesman_id, 'product_id' => $product_id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
	...
	public function getSalesmanProductCommissionByPID($product_id = false, $salesman_id = false)
    {
        $pg = "(SELECT {$this->db->dbprefix('product_commissions')}.commission as commission, {$this->db->dbprefix('product_commissions')}.product_id as product_id FROM {$this->db->dbprefix('product_commissions')} WHERE {$this->db->dbprefix('product_commissions')}.product_id = {$product_id} AND {$this->db->dbprefix('product_commissions')}.salesman_id = {$salesman_id}) GP";

        $this->db->select("{$this->db->dbprefix('products')}.id as id, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, GP.commission", FALSE)
	...
	public function updateAwardPoints($id = false, $data = false)
	{
		if($id > 0){
	...
	public function addRedeemPoint($id = false, $data = array())
    {
		$user = $this->site->getUser($id);
        if ($this->db->insert('user_redeem_points', $data)) {
			if($user){
				$remain_points = (double)$user->award_points - (double)$data['amount'];
				$this->db->update('users', array('award_points' => $remain_points), array('id' => $user->id));
			}
            return true;
        }
        return false;
    ...
    {
		if($id && $id > 0){	
			if ($this->db->delete('user_redeem_points', array('id' => $id))) {
				return true;
			}
		}
        return FALSE;
	...
	public function addShareCommission($salesman_id = false, $data = false){
		$this->db->delete("share_commission_rates",array("salesman_id"=>$salesman_id));
		if($this->db->insert_batch("share_commission_rates",$data)){
			return true;
		}
		return false;
	}
	
	public function getShareCommissions($salesman_id = false){
		$q = $this->db->get_where("share_commission_rates",array("salesman_id"=>$salesman_id));
		if($q->num_rows() > 0){
			foreach($q->result() as $row){
				$data[] = $row;
			}
