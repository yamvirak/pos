...
class Stripe_payments extends CI_Model
{
    ...
    public function create_card_token($card_info = false)
    {
    ...
    public function insert($token = false, $description = false, $amount = false, $currency = false)
    {
        try {
    ...
    function charge($token = false, $description = false, $amount = false, $currency = false)
    {
        return $this->insert($token, $description, $amount, $currency);
    }

    public function insert_many($data = false)
    {
        $ids = array();
    ...
    public function get_limit($limit = false, $offset = 0)
    {
        return $this->get_all_transactions($limit, $offset);
    }

    function refund($transaction_id = false, $amount = 'all')
    {
    ...
    function charge_to_array($charge = false)
    {
        $data = array(
            'id' => $charge->id,
    ...
    function card_to_array($card = false)
    {
        $data = array(
