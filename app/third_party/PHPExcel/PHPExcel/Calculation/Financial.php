...
class PHPExcel_Calculation_Financial {
	...
	public static function ISPMT() {
		...
		$principalRemaining = array_shift($aArgs);

		// Calculate
		$principalPayment = ($principalRemaining * 1.0) / ($numberPeriods * 1.0);
		for($i=0; $i <= $period; ++$i) {
			$returnValue = $interestRate * $principalRemaining * -1;
			$principalRemaining -= $principalPayment;
			// principal needs to be 0 after the last payment, don't let floating point screw it up
			if($i == $numberPeriods) {
				$returnValue = 0;
			}
		}
