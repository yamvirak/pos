...
class Payments extends MY_Controller
{
    ...
    function paypalipn()
    {
        ...
        $this->cus->log_payment('Paypal IPN called');
        ...
        if (!$fp) {
            ...
            $this->cus->log_payment('Paypal Payment Failed (IPN HTTP ERROR)', $errstr);
            ...
            while (!feof($fp)) {
                ...
                if (stripos($res, "VERIFIED") !== false) {
                    $this->cus->log_payment('Paypal IPN - VERIFIED');
                    ...
                    if (($_POST['payment_status'] == 'Completed' || $_POST['payment_status'] == 'Processed' || $_POST['payment_status'] == 'Pending') &&
						...
						$sales = $this->sales_model->getInvoiceByID($invoice_no);
                        if ($inv = $sales) {
                            $payment = array(
                                'date' => date('Y-m-d H:i:s'),
                                'sale_id' => $invoice_no,
                                'reference_no' => $this->site->getReference('pay',$sales->biller_id),
                                'amount' => $amount,
                                'paid_by' => 'paypal',
                            ...
                            if ($this->sales_model->addPayment($payment)) {
                                ...
                                $this->cus->send_email($paypal->account_email, 'Payment has been made via Paypal', $message);
                                $this->cus->log_payment('Payment has been made for Sale Reference #' . $_POST['item_name'] . ' via Paypal (' . $_POST['txn_id'] . ').', print_r($_POST, ture));
                            }
                        }
                    } else {

                        $this->cus->log_payment('Payment failed for Sale Reference #' . $reference . ' via Paypal (' . $_POST['txn_id'] . ').', print_r($_POST, ture));

                    }
                } else if (stripos($res, "INVALID") !== false) {
                    $this->cus->log_payment('INVALID response from Paypal. Payment failed via Paypal.', print_r($_POST, ture));
                }
            }
            fclose($fp);
        }
    ...
    function skrillipn()
    {
        ...
        $this->cus->log_payment('Skrill IPN called');
        ...
        if (strtoupper(md5($concatFields)) == $_POST['md5sig'] && $_POST['status'] == 2 && $_POST['pay_to_email'] == $skrill->account_email) {
            ...
            if ($inv = $sales) {
                ...
                if ($this->sales_model->addPayment($payment)) {
                    ...
                    $msg = file_get_contents('./themes/' . $theme . '/views/email_templates/payment.html');
                    $message = $this->parser->parse_string($msg, $parse_data);
                    $this->cus->send_email($skrill->account_email, 'Payment has been made via Skrill', $message);
                    $this->cus->log_payment('Payment has been made for Sale Reference #' . $_POST['item_name'] . ' via Skrill (' . $_POST['mb_transaction_id'] . ').', print_r($_POST, ture));
                }
            }
        } else {
            $this->cus->log_payment('Payment failed for via Skrill.', print_r($_POST, ture));
            exit;
        }
