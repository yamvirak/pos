...
class Converts extends MY_Controller
{
	...
	public function edit_convert($id) {
		...
		if ($this->form_validation->run() == true && $this->products_model->updateConvert($id, $data, $raw_materials, $finished_goods, $stockmoves, $accTrans)){
			...
			$this->core_page('converts/edit_convert', $meta, $this->data);
		}
    }
...
function boms() {
    $this->cus->checkPermissions();
    $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
    $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('converts'), 'page' => lang('converts')), array('link' => '#', 'page' => lang('boms')));
    $meta = array('page_title' => lang('boms'), 'bc' => $bc);
    $this->core_page('manufacturings/boms', $meta, $this->data);
}

function getBoms(){
    $this->cus->checkPermissions('boms');
    $delete_link = "<a href='#' class='tip po' title='<b>" . $this->lang->line("delete") . "</b>' data-content=\"<p>"
        . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete11' href='" . site_url('converts/delete_bom/$1') . "'>"
        . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fonts fa fa-trash-o\"></i></a>";
    ...
    $this->datatables
        ->select("
            ...
            boms.name, 
            boms.total_cost, 
            CONCAT({$this->db->dbprefix('users')}.last_name, ' ', {$this->db->dbprefix('users')}.first_name) as created_by,  attachment")
        ->from('boms')
    ...
    $this->datatables->add_column("Actions", "<div class='text-center'><a href='" . site_url('converts/edit_bom/$1') . "' class='tip' title='" . lang("edit") . "'><i class='fonts fa fa-edit'></i></a> " . $delete_link . "</div>", "id");
    echo $this->datatables->generate();
}
...
function add_bom(){
    ...
    if ($this->form_validation->run() == true) {
        ...
        $biller_id = $this->input->post('biller');
        if ($this->Owner || $this->Admin || $this->cus->GP['products-converts-date']) {
            $updated_at = $this->cus->fld(trim($this->input->post('updated_at')));
        } else {
            $updated_at = ($this->Settings->date_with_time==0 ? date('Y-m-d') : date('Y-m-d H:i:s'));
        }
        $i = isset($_POST['product_id']) ? sizeof($_POST['product_id']) : 0;
        for ($r = 0; $r < $i; $r++) {
            $product_id = $_POST['product_id'][$r];
        ...
        foreach ($products as $bom){
            $total_bom += $bom["total_cost"];
        }
        $data = array(
            'name' => $name,
            'total_cost' => $total_bom,
    ...
    if ($this->form_validation->run() == true && $this->settings_model->addBom($data, $products, $finished_goods)) {
        ...
        $this->core_page('manufacturings/add_bom', $meta, $this->data);
...
function edit_bom($id = false){
    ...
    if ($this->form_validation->run() == true && $this->settings_model->updateBom($id, $data, $products, $finished_goods)) {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('converts'), 'page' => lang('converts')),array('link' => site_url('converts/boms'), 'page' => lang('boms')), array('link' => '#', 'page' => lang('edit_bom')));
        $meta = array('page_title' => lang('edit_bom'), 'bc' => $bc);
        $this->session->set_userdata('remove_bomls', 1);
        $this->core_page('manufacturings/edit_bom', $meta, $this->data);
    }
}

public function Bomsuggestions()
    {
        $term = $this->input->get('term', true);
        if (strlen($term) < 1 || !$term) {
            die("<script type='text/javascript'>setTimeout(function(){ window.top.location.href = '" . site_url('welcome') . "'; }, 10);</script>");
...
public function view_boms($id)

    {
        //$this->cas->checkPermissions('converts');
        $boms = $this->products_model->getBomByID($id);

        $biller_id = $this->sales_model->getSales();
        $this->data['boms'] = $boms;
        $this->data['bom'] = $this->products_model->getBomByID($boms->bom_id);
        $this->data['biller'] = $this->site->getCompanyByID($boms->biller_id);
        $this->data['biller'] = $this->site->getCompanyByID($biller_id->biller_id);
        $this->data['bom_items'] = $this->products_model->getBomItems($id);
        $this->data['created_by'] = $this->site->getUser($boms->created_by);
        //$this->data['warehouse'] = $this->site->getWarehouseByID($convert->warehouse_id);
        if ($this->Owner || $this->Admin || $this->cus->GP['unlimited-print']){
            $this->data['print'] = 0;
        }else{
        ...
        $this->load->view($this->theme.'manufacturings/view_boms', $this->data);
    }
/*==================================================================Cnnvet Report*/
function convert_report() {
    $this->cus->checkPermissions();
    $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
    $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('converts'), 'page' => lang('converts')), array('link' => '#', 'page' => lang('convert_report')));
    $meta = array('page_title' => lang('convert_report'), 'bc' => $bc);
    $this->core_page('reports/convert_report', $meta, $this->data);
}

// function convert_detail() {
//     $this->cus->checkPermissions();
//     $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
//     $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('converts'), 'page' => lang('converts')), array('link' => '#', 'page' => lang('boms')));
//     $meta = array('page_title' => lang('boms'), 'bc' => $bc);
//     $this->core_page('reports/convert_detail', $meta, $this->data);
// }


// public function convert_detail()
// {	
//     $this->cus->checkPermissions('convert_detail');
