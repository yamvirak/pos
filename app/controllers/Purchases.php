...
class Purchases extends MY_Controller
{
    ...
    public function index($warehouse_id = null, $biller_id = NULL, $payment_status = null)
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')),array('link' => site_url('purchases'), 'page' => lang('purchase')), array('link' => '#', 'page' => lang('purchases')));
        $meta = array('page_title' => lang('purchases'), 'bc' => $bc);
        $this->core_page('purchases/index', $meta, $this->data);

    }

    public function getPurchases($warehouse_id = null, $biller_id = NULL , $payment_status = null)
    {
		if($warehouse_id == 0){
			$warehouse_id = null;
		}
		...
		$add_receive_link = '';
		if($this->config->item('receive_item')==true && ((isset($this->GP['purchases-add_receive']) && $this->GP['purchases-add_receive']) || $this->Owner || $this->Admin)){
			$add_receive_link = anchor('purchases/add_receive/$1', '<i class="fa fa-file-text-o"></i> ' . lang('add_receive_item'));
		}
		$add_freight_link = '';
		if($this->config->item('receive_item')==true){
			$add_freight_link = anchor('purchases/add_freight/$1', '<i class="fa fa-money"></i> ' . lang('add_freight'), 'data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#myModal"');
		}
        ...
        $email_link = anchor('purchases/email/$1', '<i class="fa fa-envelope"></i> ' . lang('email_purchase'), 'data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#myModal"');
        ...
        $return_link = '';
		if ((isset($this->GP['purchases-return_purchases']) && $this->GP['purchases-return_purchases']) || $this->Owner || $this->Admin) {
        ...
        <ul class="dropdown-menu pull-right" role="menu">
            <li>' . $detail_link . '</li>
			<li>' . $add_freight_link . '</li>
			<li>' . $add_receive_link . '</li>
            <li>' . $payments_link . '</li>
		...
		$this->datatables
			...
			purchases.po_reference_no,
			purchases.si_reference_no,
			projects.name,
			purchases.supplier, 
		...
		$this->datatables->join("projects","projects.id=purchases.project_id","left");
		if ($warehouse_id) {
		...
		if ($payment_status=='due') {
			$this->datatables->join('payment_terms','payment_terms.id = purchases.payment_term','inner');
		...
		$this->datatables->where('status !=', 'freight');
	...
	public function purchase_return($warehouse_id = null, $biller_id = NULL, $payment_status = null)
    {
		...
		$this->data['biller'] = $biller_id ? $this->site->getCompanyByID($biller_id) : null;
		$this->data['payment_status'] = $payment_status;
        $bc = array(array('link' => base_url(), 'page' => lang('home')),array('link' => site_url('purchases'), 'page' => lang('purchase')), array('link' => '#', 'page' => lang('purchase_returns')));
        $meta = array('page_title' => lang('purchase_returns'), 'bc' => $bc);
        $this->core_page('purchases/purchase_return', $meta, $this->data);

    }
	
	public function getPurchaseReturn($warehouse_id = null, $biller_id = NULL)
    {
		$this->cus->checkPermissions('return_purchases');
		if($warehouse_id == 0){
			$warehouse_id = null;
        ...
        $detail_link = anchor('purchases/view/$1', '<i class="fa fa-file-text-o"></i> ' . lang('purchase_details'));
        $payments_link = anchor('purchases/payment_returns/$1', '<i class="fa fa-money"></i> ' . lang('view_payments'), 'data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#myModal"');
		$add_payment_link = anchor('purchases/add_payment_return/$1', '<i class="fa fa-money"></i> ' . lang('add_payment'), 'data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#myModal"');
        $delete_link = "<a href='#' class='po' title='<b>" . $this->lang->line("delete_purchase") . "</b>' data-content=\"<p>"
        . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . site_url('purchases/delete/$1') . "'>"
        . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i> "
        . lang('delete_purchase') . "</a>";
		...
		$this->datatables
			->select("purchases.id as id, 
			DATE_FORMAT(".$this->db->dbprefix('purchases').".date, '%Y-%m-%d %T') as date, 
			purchases.return_purchase_ref,
			purchases.reference_no,
			projects.name,
			purchases.supplier, 
			abs(".$this->db->dbprefix('purchases').".grand_total) as grand_total, 
			IF(
				(round((pur_purchase.pur_total-IFNULL(pur_purchase.pur_paid,0)-abs(cus_purchases.grand_total)),2)) < 0,
				abs(round((pur_purchase.pur_total-IFNULL(pur_purchase.pur_paid,0)-abs(cus_purchases.grand_total)),2)),
				'0'
			) as credit_amount,
			abs(".$this->db->dbprefix('purchases').".paid) as paid, 
			IF(
				(round((pur_purchase.pur_total-IFNULL(pur_purchase.pur_paid,0)-abs(cus_purchases.grand_total)),2)) < 0,
				((abs(round((pur_purchase.pur_total-IFNULL(pur_purchase.pur_paid,0)-abs(cus_purchases.grand_total)),2)))-abs(".$this->db->dbprefix('purchases').".paid)),
				'0'
			) as balance,
			IF(
				(round((pur_purchase.pur_total-IFNULL(pur_purchase.pur_paid,0)-abs(cus_purchases.grand_total)),2)) < 0,
				IF(((abs(round((pur_purchase.pur_total-IFNULL(pur_purchase.pur_paid,0)-abs(cus_purchases.grand_total)),2)))-abs(".$this->db->dbprefix('purchases').".paid))=0,
					'paid',
					IF(IFNULL(".$this->db->dbprefix('purchases').".paid,0)=0,
					'pending',
					'partial'
					)
				)	
		...
		$this->datatables->join('(select id, abs(grand_total) as pur_total, abs(paid) as pur_paid FROM cus_purchases) as pur_purchase','pur_purchase.id = purchases.purchase_id','inner');
		$this->datatables->join("projects","projects.id=purchases.project_id","left");
		if ($warehouse_id) {
            $this->datatables->where('purchases.warehouse_id', $warehouse_id);
        }
    ...
    public function add($quote_id = null, $purchase_order_id = null)
    {
        $this->cus->checkPermissions();
        ...
        if ($this->form_validation->run() == true) {
			...
			$payment_term_info = $this->purchases_model->getPaymentTermsByID($payment_term);
			...
			$receive_ids = ($this->input->post("receive_ids") ? $this->input->post("receive_ids") : null);
            $total = 0;
            $product_tax = 0;
            $order_tax = 0;
            $product_discount = 0;
                ...
                if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
                    if ($item_type == 'manual') {
						...
						if($add_product==1){
							...
							$item_id = $this->purchases_model->addProduct($addProduct);	
							$item_type = 'standard';
					...
					if($serial_no!=''){
						$reactive = 0;
						$product_serial = $this->purchases_model->getProductSerial($serial_no,$product_details->id,$warehouse_id);
						if($product_serial){
					...
					//=======accounting=========//
						if($this->Settings->accounting == 1 && $status == "received"){
							if($item_type=='manual'){
								$accTrans[] = array(
									'transaction' => 'Purchases',
									'transaction_date' => $date,
									'reference' => $reference,
									'account' => $_POST['account'][$r],
									'amount' => ($subtotal),
									'narrative' => 'Product Code: '.$item_code.'#'.'Qty: '.$item_unit_quantity.'#'.'Cost: '.$real_unit_cost,
									'description' => $note,
									'biller_id' => $biller_id,
								...
								$accTrans[] = array(
									...
									'amount' => ($real_unit_cost * $item_quantity) - $pr_item_discount,
            ...
            if($this->Settings->accounting == 1){
                ...
                $accTrans[] = array(
                    ...
                    'transaction_date' => $date,
                    'reference' => $reference,
                    'account' => ($this->Settings->default_payable_account==0 ? $purchaseAcc->ap_acc : $this->input->post('payable_account')),
                    'amount' => -$grand_total,
                    'narrative' => 'Purchases',
                    'description' => $note,
                    'biller_id' => $biller_id,
				...
				if($receive_ids == null && $status != "received"){
					$accTrans[] = array(
						'transaction' => 'Purchases',
						'transaction_date' => $date,
						'reference' => $reference,
						'account' => $purchaseAcc->prepaid_acc,
						'amount' => $total,
						'narrative' => 'Purchases Prepaid',
						'description' => $note,
						'biller_id' => $biller_id,
						'project_id' => $project_id,
						'user_id' => $this->session->userdata('user_id'),
                ...
                if($shipping > 0){
                    $accTrans[] = array(
                            ...
                            'transaction_date' => $date,
                            'reference' => $reference,
                            'account' => $purchaseAcc->shipping_acc,
                            'amount' => $shipping,
                            'narrative' => 'Shipping',
                            'description' => $note,
                            'biller_id' => $biller_id,
            ...
            $data = array(
                ...
                'ap_account' => ($this->Settings->default_payable_account==0 ? $purchaseAcc->ap_acc : $this->input->post('payable_account')),
            );
			...
			if ($purchase_order_id == 1){
				$po_infomation = $this->purchase_order_model->getPurchaseOrderByID($quote_id);
				$po_reference_no = $po_infomation->reference_no;
				$data['po_reference_no'] = $po_reference_no;
				$data['purchase_order_id'] = $quote_id;
				$po_deposit = $this->input->post("po_deposit");
				if($po_deposit > 0){
					$data['po_deposit'] = $po_deposit;
					if($this->Settings->accounting == 1){
						$accTrans[] = array(
									'transaction' => 'Purchases',
									'transaction_date' => $date,
									'reference' => $reference,
									'account' => $purchaseAcc->supplier_deposit_acc,
									'amount' => -$po_deposit,
									'narrative' => 'Deposit From Purchase Order '.$po_reference_no,
									'description' => $note,
									'biller_id' => $biller_id,
									'project_id' => $project_id,
									'user_id' => $this->session->userdata('user_id'),
				...
				$data['attachment'] = $po_infomation->attachment;
			}
			if ($purchase_order_id != 1) {
				$qo_reference_no = $this->purchases_model->getQuoteByID($quote_id)->reference_no;
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPurchase($data, $products, $accTrans,'', $product_serials, false)) {
			if ($purchase_order_id != 1) {
                $this->db->update('quotes', array('status' => 'completed'), array('id' => $quote_id));
            }			
            $this->session->set_userdata('remove_pols', 1);
            $this->session->set_flashdata('message', $this->lang->line("purchase_added")." ". $reference);
		   if($this->input->post('add_pruchase_next')){
			...
			$pr = false;
			$receive_ids = $this->input->get('receive_ids');
			if($receive_ids){
				$this->data['receive_ids'] = json_decode($receive_ids);
				$quote_id = ($this->data['receive_ids'][0]?$this->data['receive_ids'][0]:0);
				$this->data['quote'] = $this->purchases_model->getReceiveByID($quote_id);
				$items = $this->purchases_model->getReceiveItemByReceives($this->data['receive_ids']);
				...
				foreach ($items as $item) {
					...
					if($item->quantity > 0){
						...
						$row->receive_id = $item->receive_id;
						...
						$row->cost = ($item->net_unit_cost + ($item->item_discount / $item->quantity));
				...
				if ($purchase_order_id == 1) {
					$this->data['quote'] = $this->purchase_order_model->getPurchaseOrderByID($quote_id);
					$this->data['po_deposit'] = $this->purchase_order_model->getTotalDeposit($quote_id)->amount;
					$items = $this->purchase_order_model->getAllPurchaseOrderItems($quote_id);
					$supplier_id = $this->data['quote']->supplier_id;				
					krsort($items);
					$c = rand(100000, 9999999);
					foreach ($items as $item) {
						if($item->quantity > $item->pu_quantity){
							$convert_unit = false;
							if($item->pu_quantity > 0){
								$item->quantity = $item->quantity - $item->pu_quantity;
							...
							if ($row->type == 'combo') {
								...
								foreach ($combo_items as $citem) {
									...
									$crow->base_unit_cost = $item->real_unit_price;
									$crow->unit = $item->product_unit_id;
									$crow->discount = $item->discount ? $item->discount : '0';
									$crow->cost = ($item->net_unit_price + ($item->item_discount / $item->quantity));
									$crow->tax_rate = $item->tax_rate_id;
									$crow->real_unit_cost = $item->real_unit_price;
									$crow->unit_cost = $row->tax_method ? $item->unit_price + ($item->item_discount / $item->quantity) + ($item->item_tax / $item->quantity) : $item->unit_price + ($item->item_discount / $item->quantity);
									$crow->expiry = '';
								...
								$row->base_quantity = $item->quantity;
								$row->base_unit = $row->unit ? $row->unit : $item->product_unit_id;
								$row->base_unit_cost = $item->real_unit_price;
								$row->unit = $item->product_unit_id;
								$row->qty = $item->unit_quantity;
								$row->option = $item->option_id;
								$row->discount = $item->discount ? $item->discount : '0';
								$row->cost = ($item->net_unit_price + ($item->item_discount / $item->quantity));
								$row->tax_rate = $item->tax_rate_id;
								$row->expiry = '';
								$row->real_unit_cost = $item->real_unit_price;
								$row->unit_cost = $row->tax_method ? $item->unit_price + ($item->item_discount / $item->quantity) + ($item->item_tax / $item->quantity) : $item->unit_price + ($item->item_discount / $item->quantity);
								$options = $this->purchases_model->getProductOptions($row->id);
								$units = $this->site->getUnitbyProduct($row->id,$row->base_unit);
								$tax_rate = $this->site->getTaxRateByID($row->tax_rate);
								$ri = $this->Settings->item_addition ? $row->id : $c;
			...
			if($this->config->item('purchase_order')){
				$this->data['purchase_orders'] = $this->site->getRefPurchaseOrders('approved');
			}
			...
			if($this->Settings->accounting == 1){
				$this->data['accounts'] = $this->site->getAccount(array('AS','LI','EQ','RE','CO','EX','OI','OX','GL'));
                if($this->Settings->default_payable_account != 0){
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')),array('link' => site_url('purchases'), 'page' => lang('purchase')), array('link' => site_url('purchases'), 'page' => lang('purchases')), array('link' => '#', 'page' => lang('add_purchase')));
            $meta = array('page_title' => lang('add_purchase'), 'bc' => $bc);
            $this->core_page('purchases/add', $meta, $this->data);
        }
    }
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
			...
			$receive_ids = ($this->input->post("receive_ids") ? $this->input->post("receive_ids") : null);
			$recieve_items = $this->purchases_model->getReceiveByPurchaseID($id);
            $total = 0;
            $product_tax = 0;
            $order_tax = 0;
            $product_discount = 0;
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_unit = $_POST['product_unit'][$r];
                $item_quantity = $_POST['product_base_quantity'][$r];
				if($this->config->item('receive_item') && $status != 'received'){
                ...
                if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity) && isset($quantity_balance)) {
					...
					if($serial_no!='' && $quantity_received > 0){
						$reactive = 0;
						$product_serial = $this->purchases_model->getProductSerial($serial_no,$product_details->id,$warehouse_id, $id);

						if($product_serial){
							if($product_serial->inactive==0){
								if($this->purchases_model->getPurchaseItemSerial($product_details->id,$id,$serial_no)){
									$reactive = 1;
								}else{
									$this->session->set_flashdata('error', lang("serial_is_existed").' ('.$serial_no.') ');
									redirect($_SERVER["HTTP_REFERER"]);
							...
							$purchase_serial = $this->purchases_model->getProductSerialByPurchaseID($serial_no,$id);
							if($purchase_serial){
								$product_serials[] = array(
									'product_id' => $product_details->id,
									'warehouse_id' => $warehouse_id,
					...
					//=======accounting=========//
						if($this->Settings->accounting == 1 && $status != "pending" && ($recieve_items == false || $receive_ids)){
							if($item_type=='manual'){
								$accTrans[] = array(
									'transaction' => 'Purchases',
									'transaction_id' => $id,
            ...
            if($this->Settings->accounting == 1){           
				...
				if($status =="pending" || ($recieve_items && $receive_ids == null)){
					$accTrans[] = array(
						'transaction' => 'Purchases',
						'transaction_id' =>$id,
						'transaction_date' => $date,
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updatePurchase($id, $data, $products, $accTrans, $product_serials)) {
            $this->session->set_userdata('remove_pols', 1);
            ...
            foreach ($inv_items as $item) {
                ...
                $row->option = $item->option_id;
                $row->real_unit_cost = $item->real_unit_cost;
				$row->unit_cost = $row->tax_method ? $item->net_unit_cost : $item->unit_cost + ($item->item_discount / $item->quantity);
                $row->cost = ($item->net_unit_cost + ($item->item_discount / $item->quantity));
                $row->tax_rate = $item->tax_rate_id;
				$row->pnote = $item->product_note;
				$row->account = $item->account_code;
                ...
                $ri = $this->Settings->item_addition ? $row->id : $c;
				if($this->Settings->product_serial == 1){
					$product_serials = $this->purchases_model->getProductSerialDetailsByProductId($row->id,$item->warehouse_id, $row->serial_no);
				}else{
				...
				if($row->code == NULL || $item->product_name == NULL){
                    $row->code = $item->product_code;
				    $row->name = $item->product_name;
				    $row->unit_cost = $item->real_unit_cost;
			...
			if($this->Settings->accounting == 1){
				$this->data['accounts'] = $this->site->getAccount(array('AS','LI','EQ','RE','CO','EX','OI','OX','GL'));
                if($this->Settings->default_payable_account != 0){
                    $this->data['payable_account'] = $this->site->getAccount('LI',$this->data['purchase']->ap_account);
                }else{
					$this->data['payable_account'] = false;
				}
			}else{
            ...
            $this->session->set_userdata('remove_pols', 1);
            $this->data['csrf'] = $this->session->userdata('user_csrf');
            $bc = array(array('link' => base_url(), 'page' => lang('home')),array('link' => site_url('purchases'), 'page' => lang('purchase')), array('link' => site_url('purchases'), 'page' => lang('purchases')), array('link' => '#', 'page' => lang('edit_purchase')));
            $meta = array('page_title' => lang('edit_purchase'), 'bc' => $bc);
            $this->core_page('purchases/edit', $meta, $this->data);
        }
    }
    ...
    public function purchase_by_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
                ...
                foreach ($final as $csv_pr) {
                    ...
                    if (isset($csv_pr['code']) && isset($csv_pr['net_unit_cost']) && isset($csv_pr['quantity'])) {
                        ...
                        if ($product_details = $this->purchases_model->getProductByCode($csv_pr['code'])) {
                            ...
                            $item_net_cost = $this->cus->formatDecimalRaw($csv_pr['net_unit_cost']);
                            ...
                            if (isset($item_discount) && $this->Settings->product_discount) {
                                ...
                                if ($dpos !== false) {
                                    $pds = explode("%", $discount);
                                    $pr_discount = $this->cus->formatDecimalRaw(((($this->cus->formatDecimalRaw($item_net_cost)) * (Float) ($pds[0])) / 100), 4);
                                } else {
                                ...
                                if ($tax_details = $this->purchases_model->getTaxRateByName($item_tax_rate)) {
                                    ...
                                    if ($tax_details->type == 1) {
                                        if (!$product_details->tax_method) {
                                            $item_tax = $this->cus->formatDecimalRaw((($item_net_cost - $pr_discount) * $tax_details->rate) / (100 + $tax_details->rate), 4);
                                            $tax = $tax_details->rate . "%";
                                            $item_net_cost -= $item_tax;
                                        } else {
                                            $item_tax = $this->cus->formatDecimalRaw((($item_net_cost - $pr_discount) * $tax_details->rate) / 100, 4);
                                            $tax = $tax_details->rate . "%";
                                        }
                                    } elseif ($tax_details->type == 2) {
                            ...
                            $product_tax += $pr_item_tax;
                            $subtotal = $this->cus->formatDecimalRaw(((($item_net_cost * $item_quantity) + $pr_item_tax) - $pr_item_discount), 4);
                            ...
                            $products[] = array(
                                ...
                                'date' => date('Y-m-d', strtotime($date)),
                                'status' => $status,
                                'unit_cost' => $this->cus->formatDecimalRaw(($item_net_cost + $item_tax), 4),
                                'real_unit_cost' => $this->cus->formatDecimalRaw(($item_net_cost + $item_tax + $pr_discount), 4),
                            );

                            $total += $this->cus->formatDecimalRaw(($item_net_cost * $item_quantity), 4);
            ...
            if ($this->Settings->tax2 != 0) {
                ...
                if ($order_tax_details = $this->site->getTaxRateByID($order_tax_id)) {
                    ...
                    if ($order_tax_details->type == 1) {
                        $order_tax = $this->cus->formatDecimalRaw((($total + $product_tax - $total_discount) * $order_tax_details->rate) / 100);
                    }
                }
            } else {
                $order_tax_id = null;
            ...
            $grand_total = $this->cus->formatDecimalRaw(($total + $total_tax + $this->cus->formatDecimalRaw($shipping) - $total_discount), 4);
            $data = array('reference_no' => $reference,
                'date' => $date,
                'supplier_id' => $supplier_id,
                'supplier' => $supplier,
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPurchase($data, $products)) {
            ...
            $this->data['warehouses'] = $this->site->getWarehouses();
            $this->data['tax_rates'] = $this->site->getAllTaxRates();
            $bc = array(array('link' => base_url(), 'page' => lang('home')),array('link' => site_url('purchases'), 'page' => lang('purchase')), array('link' => site_url('purchases'), 'page' => lang('purchases')), array('link' => '#', 'page' => lang('add_purchase_by_csv')));
            $meta = array('page_title' => lang('add_purchase_by_csv'), 'bc' => $bc);
            $this->core_page('purchases/purchase_by_csv', $meta, $this->data);
    ...
    public function expenses($warehouse_id = null,$biller_id = NULL)
    {
        $this->cus->checkPermissions();
		$this->data['warehouses'] = $this->site->getWarehouses();
		$this->data['warehouse'] = $warehouse_id ? $this->site->getWarehouseByID($warehouse_id) : null;
        ...
        $this->core_page('purchases/expenses', $meta, $this->data);
    }

    public function getExpenses($warehouse_id = NULL, $biller_id = NULL)
    {
        $this->cus->checkPermissions('expenses');
		$payments_link = anchor('purchases/payments/0/$1', '<i class="fa fa-money"></i> ' . lang('view_payments'), ' class="expense_payment" data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#myModal"');
		$add_payment_link = anchor('purchases/add_payment/0/$1', '<i class="fa fa-money"></i> ' . lang('add_payment'), 'class="expense_payment" data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#myModal"');
        $detail_link = anchor('purchases/expense_note/$1', '<i class="fa fa-file-text-o"></i> ' . lang('expense_note'), 'data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#myModal2"');
        $edit_link = anchor('purchases/edit_expense/$1', '<i class="fa fa-edit"></i> ' . lang('edit_expense'), ' class="edit_expense" ');
		...
		$approve_link = "";
		$unapprove_link = "";
		if($this->Settings->approval_expense==1 && ($this->Admin || $this->Owner || $this->GP['purchases-approve_expense'])){
			...
			. lang('r_u_sure') . "</p><a class='btn btn-success po-delete' href='" . site_url('purchases/unapprove_expense/$1') . "'>"
			. lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-check\"></i> "
	...
	public function add_expense(){
		...
		if ($this->form_validation->run() == true && $this->purchases_model->addExpense($data,$items, $accTrans,$payment)) {	
			...
			if($this->input->post('add_expense_next')){
				redirect('purchases/add_expense');
			}else{
            ...
            $this->core_page('purchases/add_expense', $meta, $this->data);
        }
	}
	...
	public function edit_expense($id = null){
		...
		if ($this->form_validation->run() == true && $this->purchases_model->updateExpense($id,$data,$items,$accTrans,$payment)) {	
            ...
            $this->core_page('purchases/edit_expense', $meta, $this->data);
        }
	}
