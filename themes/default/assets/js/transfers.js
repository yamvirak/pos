...
if (to_biller = localStorage.getItem('to_biller')) {
    $('#to_biller').select2("val", to_biller);
}
$('#to_biller').change(function (e) {
    localStorage.setItem('to_biller', $(this).val());
});
if (fr_biller = localStorage.getItem('fr_biller')) {
    $('#fr_biller').select2("val", fr_biller);
}
$('#fr_biller').change(function (e) {
    localStorage.setItem('fr_biller', $(this).val());
});
if (from_warehouse = localStorage.getItem('from_warehouse')) {
    ...
    //$(document).on('change', '#tonote', function (e) {
		...
		$(document).on("change", '.rexpired', function () {
			...
			toitems[item_id].row.expired = new_expired;
...
$('body').bind('keypress', function (e) {
	...
	$(document).on('change', '.sunit', function () {
		...
		toitems[item_id].row.base_quantity = base_quantity;
		toitems[item_id].row.unit = new_unit;
		localStorage.setItem('toitems', JSON.stringify(toitems));
        loadItems();
    });
	...
	$(document).on("focus", '.rserial', function () {
        ...
        toitems[item_id].row.serial = new_serial;
        localStorage.setItem('toitems', JSON.stringify(toitems));
        loadItems();
    });
