...
$('body a, body button').attr('tabindex', -1);
    ...
    $('#reset').click(function (e) {
            bootbox.confirm(lang.r_u_sure, function (result) {
                if (result) {
                    ...
                    if (localStorage.getItem('rtleavetypes')) {
                        localStorage.removeItem('rtleavetypes');
                    }
    ...
    var $customer = $('#rctcustomer');
    $customer.change(function (e) {
        localStorage.setItem('rctcustomer', $(this).val());
    });
    if (rctcustomer = localStorage.getItem('rctcustomer')) {
        $customer.val(rctcustomer).select2({
...
$('#rtdiscount').focus(function () {
     ...
     * Show manual item addition modal
     ...
     $(document).on('click', '#addManually', function (e) {
        if (count == 1) {
            rtitems = {};
            if ($('#rtwarehouse').val() && $('#rctcustomer').val()) {
                $('#rctcustomer').select2("readonly", true);
    ...
    $(document).on("click", '#removeReadonly', function () {
       $('#rctcustomer').select2('readonly', false);
...
function loadItems() {
    ...
    if (localStorage.getItem('rtitems')) {
        ...
        $.each(sortedItems, function () {
            ...
            var service_types_select = '<select name="service_types[]"  class="form-control select service_types" style="width:100%;">';
        ...
        if(count > 1){
            $('#rctcustomer').prop("readonly", true);
            $('#rtwarehouse').select2("readonly", true);
        }else{
            $('#rctcustomer').prop("readonly", false);
            $('#rtwarehouse').select2("readonly", false);
        }
        set_page_focus();
    }
