                    ...
                    <?php if (!empty($payments)) {
                        foreach ($payments as $payment) { ?>
                            <tr class="row<?= $payment->id ?>">
                                ...
                                <td class="text-right"><?= $this->cus->formatMoney($payment->amount) ?></td>
                                <td class="text-right"><?= $this->cus->formatMoney($payment->discount) ?></td>
                                <td><?= lang($payment->paid_by); ?></td>
                                <td>
                                    <div class="text-center">
                                        <a href="<?= site_url('purchases/payment_note/' . $payment->id) ?>" data-toggle="modal" data-target="#myModal2"><i class="fonts fa fa-print"></i></a>
                                        <?= (($payment->attachment) ? '<a href="' . site_url('welcome/download/' . $payment->attachment) . '"><i class="fonts fa fa-chain"></i></a>' : '') ?>
                                        <?php if($payment->transaction != "PO Deposit"){ ?>
                                            <a href="<?= site_url('purchases/edit_payment/' . $payment->id) ?>" data-toggle="modal" data-target="#myModal2"><i class="fonts fa fa-edit"></i></a>
