...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        <?php if ($inv) { ?>
        localStorage.setItem('prdate', '<?= date($dateFormats[($Settings->date_with_time == 0 ? 'php_sdate' : 'php_ldate')], strtotime($inv->date))?>');
        localStorage.setItem('prbiller', '<?=$inv->biller_id?>');
        localStorage.setItem('prsupplier', '<?=$inv->supplier_id?>');
        localStorage.setItem('prref', '<?=$inv->reference_no?>');
        localStorage.setItem('prwarehouse', '<?=$inv->warehouse_id?>');
        localStorage.setItem('prstatus', '<?=$inv->status?>');
		localStorage.setItem('prnote', '<?= str_replace(array("\r", "\n", "'"), "", $this->cus->decode_html($inv->note)); ?>');
        localStorage.setItem('prdiscount', '<?=$inv->order_discount_id?>');
        localStorage.setItem('prtax2', '<?=$inv->order_tax_id?>');
        localStorage.setItem('prshipping', '<?=$inv->shipping?>');
        localStorage.setItem('pritems', JSON.stringify(<?=$inv_items;?>));
        <?php } ?>
        <?php if ($Owner || $Admin || $GP['purchase_requests-date']) { ?>
        $(document).on('change', '#prdate', function (e) {
            localStorage.setItem('prdate', $(this).val());
        });
        if (prdate = localStorage.getItem('prdate')) {
            $('#prdate').val(prdate);
        }
        <?php } ?>
        $(document).on('change', '#prbiller', function (e) {
            localStorage.setItem('prbiller', $(this).val());
        });
        if (prbiller = localStorage.getItem('prbiller')) {
            $('#prbiller').val(prbiller);
        }
        ItemnTotals();
        $("#add_item").autocomplete({
            source: function (request, response) {
                $.ajax({
                    type: 'get',
                    url: '<?= site_url('purchase_requests/suggestions'); ?>',
                    dataType: "json",
                    data: {
                        term: request.term,
                        warehouse_id: $("#prwarehouse").val(),
                        supplier_id: $("#prsupplier").val()
                    },
        ...
        $(window).bind('beforeunload', function (e) {
            $.get('<?= site_url('welcome/set_data/remove_prls/1'); ?>');
            if (count > 1) {
        ...
        $('#edit_purchase_request').click(function () {
            $(window).unbind('beforeunload');
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-edit"></i><?= lang('edit_purchase_request'); ?></h2>
    </div>
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                echo form_open_multipart("purchase_requests/edit/" . $id, $attrib)
                ?>
                    ...
                    <div class="col-lg-12">
                        ...
                        <?php if ($Owner || $Admin || !$this->session->userdata('biller_id')) { ?>
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    echo form_dropdown('biller', $bl, (isset($_POST['biller']) ? $_POST['biller'] : $inv->biller_id), 'id="prbiller" data-placeholder="' . $this->lang->line("select") . ' ' . $this->lang->line("biller") . '" required="required" class="form-control input-tip select" style="width:100%;"');
                                    ?>
                                </div>
                            </div>
                        <?php } else {
                        ...
                        <div class="col-md-4">
							<div class="form-group">
								...
								echo form_dropdown('warehouse', $wh, (isset($_POST['warehouse']) ? $_POST['warehouse'] : $inv->warehouse_id), 'id="prwarehouse" class="form-control input-tip select" data-placeholder="' . $this->lang->line("select") . ' ' . $this->lang->line("warehouse") . '" required="required" style="width:100%;" ');
								?>
							</div>
						</div>
                        ...
                        <div class="col-md-4 hidden">
                            <div class="form-group">
                                <?= lang("discount", "prdiscount"); ?>
                                <?php echo form_input('discount', '', 'class="form-control input-tip" id="prdiscount" '.(($Owner || $Admin || $this->session->userdata('allow_discount')) ? '' : 'readonly="true"')); ?>
                            </div>
                        </div>
                        <?php } ?>
                        ...
                        <div class="col-md-4 hidden">
                            <div class="form-group">
                                <?= lang("shipping", "prshipping"); ?>
                                <?php echo form_input('shipping', '', 'class="form-control input-tip" id="prshipping"'); ?>
                        ...
                        <div class="col-md-4 hidden">
                            <div class="form-group">
                                <?= lang("status", "prstatus"); ?>
                                <?php $st = array('pending' => lang('pending'), 'sent' => lang('sent'), 'completed' => lang('completed'));
                                echo form_dropdown('status', $st, '', 'class="form-control input-tip" id="prstatus"'); ?>
                        ...
                        <div class="col-sm-12">
                            <div
                                class="fprom-group"><?php echo form_submit('edit_purchase_request', $this->lang->line("submit"), 'id="edit_purchase_request" class="btn btn-primary" style="padding: 6px 15px; margin:15px 0;"'); ?>
