...
<div class="modal-dialog modal-lg">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title" id="myModalLabel"><?= lang('agency_payments').' ('.lang('sale').' '.lang('reference').': '.$inv->reference_no.')'; ?></h4>
        </div>
                    ...
                    <?php if (!empty($payments)) {
                        foreach ($payments as $payment) { ?>
                            <tr class="row<?= $payment->id ?>">
                                ...
                                <td style="text-align:right !important"><?= $this->cus->formatMoney($payment->amount + $payment->interest_paid + $payment->penalty_paid)  ; ?></td>
								<td class="text-right"><?= $this->cus->formatMoney($payment->discount); ?></td>
                                ...
                                <td>
                                    <div class="text-center">
                                        ...
                                        <?php if ($payment->paid_by != 'gift_card') { ?>
                                            <a href="<?= site_url('sales/edit_agency_payment/' . $payment->id) ?>"
                                               data-toggle="modal" data-target="#myModal2"><i
                                                    class="fa fa-edit"></i></a>
                                            <a href="#" class="po"
                                               title="<b><?= $this->lang->line("delete_payment") ?></b>"
                                               data-content="<p><?= lang('r_u_sure') ?></p><a class='btn btn-danger' id='<?= $payment->id ?>' href='<?= site_url('sales/delete_agency_payment/' . $payment->id) ?>'><?= lang('i_m_sure') ?></a> <button class='btn po-close'><?= lang('no') ?></button>"
