...
<script type="text/javascript">
    ...
    $(document).ready(function () {
		...
		$('#si_reference').on('change',function(){
			...
			location.replace(site.base_url+"sales/return_sale/"+si_reference);
		});
		
        <?php if ($inv) { ?>
        //localStorage.setItem('redate', '<?= $this->cus->hrld($inv->date) ?>');
		localStorage.removeItem('redate');
        localStorage.setItem('reref', '<?= $reference ?>');
        localStorage.setItem('renote', '<?= $this->cus->decode_html($inv->note); ?>');
		...
		localStorage.setItem('return_surcharge_interest', '0');
        <?php }
			if($return){ ?>
				localStorage.setItem('redate', '<?= $this->cus->hrld($return->date) ?>');
				localStorage.setItem('reref', '<?= $return->return_sale_ref ?>');
                localStorage.setItem('reshipping', '<?= abs($return->shipping) ?>');
				localStorage.setItem('renote', '<?= $this->cus->decode_html($return->note); ?>');
				localStorage.setItem('return_surcharge', '<?= $return->surcharge ?>');
		...
		if (return_surcharge_interest = localStorage.getItem('return_surcharge_interest')) {
            $('#return_surcharge_interest').val(return_surcharge_interest);
        }
        ...
        $('#gccustomer').val(<?= ($inv ? $inv->customer_id : 0)?>).select2({
		...
		$(document).on('change', '.punit', function () {
			...
			if((qty * unit_qty) > reitems[item_id].row.quantity) {
                reitems[item_id].row.qty = 0;
            }
			
			reitems[item_id].row.unit = unit,
			reitems[item_id].row.return_unit = unit,
        ...
        $(document).on("focus", '.rquantity', function () {
			...
			var unit_qty = $('option:selected', row.find('.punit')).attr('unit_qty');
            var item_id = row.attr('data-item-id');
			var balance_qty = reitems[item_id].row.quantity;
            if (!is_numeric(new_qty) || ((new_qty * unit_qty) > balance_qty)) {
                $(this).val(old_row_qty);
                bootbox.alert('<?= lang('unexpected_value'); ?>');
                return false;
            }
            if((new_qty * unit_qty) > balance_qty) {
                bootbox.alert('<?= lang('unexpected_value'); ?>');
                $(this).val(old_row_qty);
                return false;
            }
		...
		var old_surcharge_interest;
        $(document).on("focus", '#return_surcharge_interest', function () {
            old_surcharge_interest = $(this).val() ? $(this).val() : '0';
        }).on("change", '#return_surcharge_interest', function () {
            var new_surcharge_interest = $(this).val() ? $(this).val() : '0';
            if (!is_valid_discount(new_surcharge_interest)) {
                $(this).val(old_surcharge_interest);
                bootbox.alert('<?= lang('unexpected_value'); ?>');
                return;
            }
            localStorage.setItem('return_surcharge_interest', new_surcharge_interest);
            loadItems();
        });

        var old_shipping;
        $(document).on("focus", '#reshipping', function () {
            old_shipping = $(this).val() ? $(this).val() : 0;
        }).on("change", '#reshipping', function () {
            var new_shipping = $(this).val() ? ($(this).val() - 0) : 0;
            var inv_shipping = <?= ($inv ? $inv->shipping : 0)  ?>;
            if (!is_numeric(new_shipping) || new_shipping > inv_shipping) {    
                $(this).val(old_shipping);
                bootbox.alert('<?= lang('unexpected_value'); ?>');
                return;
            }
            localStorage.setItem('reshipping', JSON.stringify(new_shipping));
            loadItems();
        });
    ...
    function loadItems() {
        ...
        if (localStorage.getItem('reitems')) {
            ...
            $.each(reitems, function () {
                ...
                var item_type = item.row.type, cost=item.row.cost, product_id = item.row.id, combo_items = item.combo_items, sale_item_id = item.row.sale_item_id, item_option = item.row.option, item_price = item.row.price, item_qty = item.row.qty, item_aqty = item.row.quantity, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_option = item.row.option, item_code = item.row.code, item_serial = item.row.serial, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
                var unit_price = item.row.base_price;
                var product_unit = item.row.unit, base_quantity = item.row.base_quantity;

				var s_unit = $("<select id=\"unit\" name=\"unit\[\]\" class=\"form-control select punit\" />");
				if(item.units !== false) {
					$.each(item.units, function () {
						if (item.row.return_unit == this.id){
							$("<option />", {value: this.id, text: this.name, selected: 'selected', unit_qty: this.operation_value}).appendTo(s_unit);
							base_quantity = formatDecimal(unitToBaseQty(item.row.qty, this), 4);
							unit_price = unit_price * this.operation_value;
	
							if(item.row.item_discount > 0){
								item_discount = item.row.item_discount * this.operation_value;
							}
							
						}else{
							$("<option />", {value: this.id, text: this.name, unit_qty: this.operation_value}).appendTo(s_unit);
						}
					});
				} else {
					$("<option />", {value: 0, text: 'n/a'}).appendTo(s_unit);
					s_unit = s_unit.hide();
				}
				...
				if (site.settings.product_expiry == 1) {
                    tr_html += '<td class="text-center"><input type="hidden" value="'+item.row.expiry+'" name="expiry[]" />'+item.row.expiry+'</td>';
                }
				tr_html += '<td class="text-right"><input class="form-control input-sm text-right rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + item_price + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + item.row.real_unit_price + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatMoney(item.row.unit_price) + '</span></td>';
				tr_html += '<td class="text-center"><span>' + formatDecimal(item.row.unit_qty) +' '+item.row.unit_name + '</span></td>';
                
				tr_html += '<td><input type="hidden" name="salesman_id[]" value="'+item.row.salesman_id+'" /><input type="hidden" name="salesman[]" value="'+item.row.salesman+'" /><input type="hidden" name="cost[]" value="'+cost+'" /><input class="form-control text-center rquantity" name="quantity[]" type="text" value="' + formatDecimal(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();"><input name="product_unit[]" type="hidden" class="runit" value="' + product_unit + '"><input name="product_base_quantity[]" type="hidden" class="rbase_quantity" value="' + base_quantity + '"></td>';
				if (site.settings.foc == 1) {
					tr_html += '<td><input class="form-control text-center foc" name="foc[]" type="text" value="' + (item.row.foc > 0 ? item.row.foc : 0) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="foc_' + row_no + '" ></td>';
				}
				tr_html += '<td>'+(s_unit.get(0).outerHTML)+'</td>';
				tr_html += '<td class="text-center"><select id=\"return_stock\" name=\"return_stock[]\" class=\"form-control select\"><option '+(item.row.return_stock=='0'?'selected':'')+' value="0">'+lang.yes+'</opion><option '+(item.row.return_stock=='1'?'selected':'')+' value="1">'+lang.no+'</opion></select></td>';
				if (site.settings.product_serial == 1) {
                    tr_html += '<td class="text-right"><input readonly class="form-control input-sm rserial" name="serial[]" type="text" id="serial_' + row_no + '" value="' + item_serial + '"></td>';
                }
                ...
                total += parseFloat((parseFloat(item_price / currency_rate) + parseFloat(pr_tax_val / currency_rate)) * parseFloat(item_qty));
            ...
            if (rediscount = localStorage.getItem('rediscount')) {
				...
				order_discount = parseFloat((total * discount_rate) / 100);
            ...
            shipping = localStorage.getItem('reshipping') - 0;
			...
			var surcharge_interest = 0;
			if (return_surcharge_interest = localStorage.getItem('return_surcharge_interest')) {
                surcharge_interest = return_surcharge_interest;
            }
			...
			$('#trsc').text(formatMoney(surcharge_interest));
            $('#trsh').text(formatMoney(shipping));
			...
			$('#return_total').val(formatDecimal(gtotal));
            <?php if(isset($return) && $return->paid <> 0) { echo "$('#amount_1').val(formatDecimal(".abs($return->paid)."));"; } ?>
    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                echo form_open_multipart("sales/return_sale/" . ($inv ? $inv->id : ''), $attrib)
                ?>

                <div class="row">
                    <div class="col-lg-12">
					
						<?php if($Settings->installment==1 && (isset($inv->installment) && $inv->installment > 0)){ ?>
							<div class="col-md-12">
								<div class="alert alert-warning">
									<button data-dismiss="alert" class="close" type="button">×</button>
									<?= lang("invoice_has_installment_payments"); ?> : <?= $installment->reference_no ?>
								</div>
							</div>
						<?php } ?>
						...
						<div class="col-md-4">
							<div class="form-group">
								...
								echo form_dropdown('si_reference', $sa_opts, ($inv ? $inv->id: ''), 'id="si_reference" class="form-control input-tip select" style="width:100%;" ');
								?>
							</div>
						</div>
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <?php echo form_input('shipping', (isset($_POST['shipping']) ? $_POST['shipping'] : ($inv ? $inv->shipping : '')), 'class="form-control input-tip text-right" id="reshipping"'); ?>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
						...
						<?php if($Settings->installment==1){ ?>
							<div class="col-md-4">
								<div class="form-group">
									<?= lang("return_surcharge_interest", "return_surcharge_interest"); ?>
									<?php echo form_input('return_surcharge_interest', (isset($_POST['return_surcharge_interest']) ? $_POST['return_surcharge_interest'] : 0), 'class="form-control input-tip" id="return_surcharge_interest"'); ?>
								</div>
							</div>
                            ...
                            <div class="control-group table-group">
                                ...
                                <div class="controls table-controls">
                                    <table id="reTable"
                                        ...
                                        <thead>
                                        <tr>
                                            <th class="col-md-3"><?= lang("product_name") . " (" . $this->lang->line("product_code") . ")"; ?></th>
											...
											<?php } ?>
											<th><?= lang("unit"); ?></th>
											<th class="col-md-1"><?= lang("return_stock"); ?></th>
                                            <?php
                            ...
                            <div id="bottom-total" class="well well-sm" style="margin-bottom: 0;">
                                <table class="table table-bordered table-condensed" style="margin-bottom:0;">
                                    <tr class="warning">
                                        ...
                                        <td>
                                            <?= lang('shipping') ?>
                                            <span class="totals_val pull-right" id="trsh">0.00</span>
                                        </td>
                                        <td>
                                            <?= lang('surcharges') ?>
										...
										<?php if($Settings->installment==1){ ?>
											<td>
                                            <?= lang('return_surcharges') ?>
                                            <span class="totals_val pull-right" id="trsc">0.00</span>
                                        </td>
										 <?php } ?>
										<td>
                                            <?= lang('order_discount') ?>
                                        ...
                                        <td>
                                            <?= lang('return_amount') ?>
											<input type="hidden" id="return_total" />
						...
						<input type="hidden" name="discount_rate" value="<?= ($inv ? ((100*$inv->order_discount)/$inv->total) : 0 )  ?>" id="discount_rate" required="required"/>
