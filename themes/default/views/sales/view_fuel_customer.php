		...
		<div class="modal-body">
						...
						<th>
							<table>
								<tr>
									...
									<td class="text_center" style="width:20%">
										<?= $this->cus->qrcode('link', urlencode(site_url('sales/view_fuel_customer/' . $fuel_customer->id)), 2); ?>
									</td>
								</tr>
							</table>
						</th>
						...
						<th>
							<table>
								<tr>
									...
									<td class="text_center" style="width:30%"><span style="font-size:<?= $font_size+5 ?>px"><b><i><?= lang('fuel_customer') ?></i></b></span></td>
										...
										<fieldset style="margin-left:5px !important">
											...
											<table>
												<tr>
													...
													<td style="text-align:left"> : <b><?= $fuel_customer->reference ?></b></td>
												</tr>
												<tr>
													<td><?= lang('date') ?></td>
													<td style="text-align:left"> : <?= $this->cus->hrsd($fuel_customer->date) ?></td>
												</tr>		
												<tr>
													<td><?= lang('salesman') ?></td>
													<td style="text-align:left"> : <b><?= $salesman->last_name." ".$salesman->first_name ?></b></td>
												</tr>
											</table>
										</fieldset>
									</td>
					...
					<?php
						...
						foreach ($fuel_customer_items as $fuel_customer_item){
							...
							if($this->config->item("customer_truck")){
								$truck = '<td class="text_left">'.($fuel_customer_item->truck_name ? $fuel_customer_item->truck_name .' - '.$fuel_customer_item->plate_number : '').'</td>';
							}
							$tbody .='<tr>
											<td class="text_center">'.$i.'</td>
											<td class="text_center">'.$fuel_customer_item->product_code.'</td>
											<td class="text_left">'.$fuel_customer_item->product_name .'</td>
											'.$truck.'
											<td class="text_center">'.$this->cus->formatQuantity($fuel_customer_item->quantity).'</td>
											<td class="text_right">'.$this->cus->formatMoney($fuel_customer_item->unit_price).'</td>
											<td class="text_right">'.$this->cus->formatMoney($fuel_customer_item->subtotal).'</td>
										</tr>';		
							$i++;
						}
						$footer_colspan = 4;
						...
						$footer_rowspan = 1;
						$tfooter = '';
						$footer_note = '<td class="footer_des" rowspan="'.$footer_rowspan.'" colspan="'.$footer_colspan.'">'.$this->cus->decode_html($fuel_customer->note).'</td>';
						$tfooter .= '<tr>
										'.$footer_note.'
										<td class="text_right"><b>'.lang("grand_total").'</b></td>
										<td class="text_right"><b>'.$this->cus->formatMoney($fuel_customer->grand_total).'</b></td>
									</tr>';
					?>
					<tr>
						<td>
					...
					<tr class="tr_print">
						<td>
							<table style="margin-top:<?= $margin_signature ?>px;">
								<tr>
									<td class="text_center" style="width:50%"><?= lang("seller") .' '. lang("signature") ?></td>
			...
			<div id="buttons" style="padding-top:10px;" class="no-print">
				...
				<div class="btn-group btn-group-justified">
					...
					<?php if ($fuel_customer->attachment) { ?>
						<div class="btn-group">
							<a href="<?= site_url('assets/uploads/' . $fuel_customer->attachment) ?>" class="tip btn btn-primary" target="_blank" title="<?= lang('attachment') ?>">
