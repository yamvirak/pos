...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-plus"></i><?= lang('edit_sale_concrete'); ?></h2>
    </div>
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <?php
					...
					echo form_open_multipart("sales/edit_sale_concrete/".$sale->id, $attrib);
                ?>
                <div class="row">
					<div class="col-md-12">
						<?php if ($Owner || $Admin || $GP['sales-date']) { ?>
							<div class="col-md-4">
								<div class="form-group">
									<?= lang("date", "csdate"); ?>
									<?php echo form_input('date', $this->cus->hrld($sale->date), 'class="form-control input-tip datetime" id="csdate" required="required"'); ?>
								</div>
							</div>
						<?php } ?>
						<div class="col-md-4 <?= ((!$Owner && !$Admin && !$GP['reference_no']) ? 'hidden' : '') ?>">
                            <div class="form-group">
                                ...
                                <?php echo form_input('reference_no', $sale->reference_no, 'class="form-control input-tip" id="csref"'); ?>
                            </div>
                        </div>
						<?php if ($Owner || $Admin || !$this->session->userdata('biller_id')) { ?>
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    echo form_dropdown('biller', $bl, $sale->biller_id, 'id="csbiller" data-placeholder="' . $this->lang->line("select") . ' ' . $this->lang->line("biller") . '" required="required" class="form-control input-tip select" style="width:100%;"');
                                    ?>
                                </div>
                            </div>
                        <?php } else {
						...
						<div class="col-md-4">
							<div class="form-group">
								...
								echo form_dropdown('warehouse', $wh, $sale->warehouse_id, 'id="cswarehouse" class="form-control input-tip select" data-placeholder="' . $this->lang->line("select") . ' ' . $this->lang->line("warehouse") . '" required="required" style="width:100%;" ');
								?>
							</div>
						</div>
						<div class="col-md-12">
                                ...
                                <div class="panel-body" style="padding: 5px;">
                                    <div class="col-md-4">
										<div class="form-group">
											...
											echo form_dropdown('customer', $cs, $sale->customer_id, 'id="cscustomer" class="form-control input-tip select" data-placeholder="' . $this->lang->line("select") . ' ' . $this->lang->line("customer") . '" required="required" style="width:100%;" ');
											?>
										</div>
                                    </div>
									<div class="col-md-4">
									...
									<div class="col-md-4">
										<div class="form-group">
											...
											<?php echo form_input('from_date', $this->cus->hrld($sale->from_date), 'class="form-control input-tip datetime" id="csfrom_date" required="required"'); ?>
										</div>
									</div>
									<div class="col-md-4">
										<div class="form-group">
											<?= lang("to_date", "csto_date"); ?>
											<?php echo form_input('to_date', $this->cus->hrld($sale->to_date), 'class="form-control input-tip datetime" id="csto_date" required="required"'); ?>
										</div>
									</div>
                                </div>
                            </div>
                    ...
                    <div class="col-lg-12">
											...
											<?php
												...
												if($sale_items){
													foreach($sale_items as $sale_item){
														$tbody .= "<tr class='con_sale_link' id='".$sale_item->con_sale_id."'>
																	<td class='text-center'><input class='con_sale_id' name='con_sale_id[]' value='".$sale_item->con_sale_id."' type='hidden'/><input class='con_date' name='con_date[]' value='".$this->cus->hrsd($sale_item->date)."' type='hidden'/>".$this->cus->hrsd($sale_item->date)."</td>
																	<td><input class='con_reference' name='con_reference[]' value='".$sale_item->reference."' type='hidden'/>".$sale_item->reference."</td>
																	<td class='text-right'><input class='con_total' name='con_total[]' value='".$sale_item->total."' type='hidden'/>".$this->cus->formatMoney($sale_item->total)."</td>
																	<td class='text-right'><input class='con_truck_charge' name='con_truck_charge[]' value='".$sale_item->truck_charge."' type='hidden'/>".$this->cus->formatMoney($sale_item->truck_charge)."</td>
																	<td class='text-right'><input class='con_pump_charge' name='con_pump_charge[]' value='".$sale_item->pump_charge."' type='hidden'/>".$this->cus->formatMoney($sale_item->pump_charge)."</td>
																	<td class='text-right'><input class='con_subtotal' name='con_subtotal[]' value='".$sale_item->subtotal."' type='hidden'/>".$this->cus->formatMoney($sale_item->subtotal)."</td>
						...
						<?php if ($Owner || $Admin || $this->session->userdata('allow_discount')) { ?>
							<div class="col-md-2">
								<div class="form-group">
									...
									<?php echo form_input('order_discount', $sale->order_discount_id, 'class="form-control input-tip" id="csdiscount"'); ?>
								</div>
							</div>
						<?php } if ($Settings->tax2) { ?>
                            <div class="col-md-2">
                                <div class="form-group">
                                    ...
                                    echo form_dropdown('order_tax', $tr, (isset($_POST['tax2']) ? $_POST['tax2'] : $sale->order_tax_id), 'id="cstax2" data-placeholder="' . $this->lang->line("select") . ' ' . $this->lang->line("order_tax") . '" required="required" class="form-control input-tip select" style="width:100%;"');
                                    ?>
                                </div>
                            </div>
                         <?php } ?>
						...
						<div class="col-sm-2">
                            <div class="form-group">
                                ...
                                echo form_dropdown('payment_term', $pt, (isset($_POST['payment_term']) ? $_POST['payment_term'] : $sale->payment_term), 'class="form-control input-tip" id="cspayment_term"'); ?>
                            ...
                            <div class="col-sm-12">
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <?php echo form_textarea('note', (isset($_POST['note']) ? $_POST['note'] : str_replace(array("\r", "\n"), "", $this->cus->decode_html($sale->note))), 'class="form-control" id="csnote" style="margin-top: 10px; height: 100px;"'); ?>
                                    </div>
                                </div>
								<div class="col-sm-6">
                                    <div class="form-group">
                                        <?= lang("staff_note", "cssnote"); ?>
                                        <?php echo form_textarea('staff_note', (isset($_POST['staff_note']) ? $_POST['staff_note'] : str_replace(array("\r", "\n"), "", $this->cus->decode_html($sale->staff_note))), 'class="form-control" id="cssnote" style="margin-top: 10px; height: 100px;"'); ?>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-12">
                            <div class="fprom-group">
								<?php echo form_submit('edit_sale_concrete', $this->lang->line("submit"), 'id="edit_sale_concrete" class="btn btn-primary" style="padding: 6px 15px; margin:15px 0;"'); ?>
							</div>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		if (customer_id > 0) {
			var location_id = "<?= $sale->location_id ?>";
			$.ajax({
					...
					if (data.locations != false) {
						$.each(data.locations, function () {
							if(this.id == location_id){
								location_sel += "<option selected value='"+this.id+"'>"+this.name+"</option>";
							}else{
		...
		$(document).on("change", "#csbiller", function () {	
			var biller = $("#csbiller").val();
			var project = '<?= $sale->project_id ?>';
			$.ajax({
				url : "<?= site_url("sales/get_project") ?>",
