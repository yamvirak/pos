...
<ul id="myTab" class="nav nav-tabs">
	<li><a href="#redeem_points" class="tab-grey"><?= lang('redeem_points') ?></a></li>
</ul>
<div class="tab-content">
	<div id="redeem_points" class="tab-pane fade in">
		<script>
			$(document).ready(function () {
				oTable = $('#table_redeem_points').dataTable({
					"aaSorting": [[1, "asc"]],
					"aLengthMenu": [[-1], ["<?= lang('all') ?>"]],
					"iDisplayLength": -1,
					'bProcessing': true, 'bServerSide': true,
					'sAjaxSource': '<?= site_url('sales/getRedeemPoints/'.$id) ?>',
				...
				$('#pdf').click(function (event) {
					...
					window.location.href = "<?=site_url('sales/redeem_points_actions/'.$id.'/1/0')?>";
					return false;
				});
				$('#xls').click(function (event) {
					event.preventDefault();
					window.location.href = "<?=site_url('sales/redeem_points_actions/'.$id.'/0/1')?>";
					return false;
				});
			});
		</script>
		<div class="box">
			<div class="box-header">
				<h2 class="blue"><i class="fa fa-heart-o"></i><?= lang('redeem_points'); ?></h2>
				<div class="box-icon">
					<ul class="btn-tasks">
						<li class="dropdown">
				...
				<div class="row">
					<div class="col-lg-12">
						...
						<div class="table-responsive">
							<table id="table_redeem_points" cellpadding="0" cellspacing="0" border="0" class="table table-bordered table-hover table-striped">
								<thead>
								<tr class="active">
