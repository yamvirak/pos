                    ...
                    <?php if (!empty($deposits)) {
                        foreach ($deposits as $deposit) { ?>
                            <tr class="row<?= $deposit->id ?>">
                                ...
                                <td style="text-align:right !important"><?= $this->cus->formatMoney($deposit->amount) . ' ' . (($deposit->attachment) ? '<a href="' . site_url('welcome/download/' . $deposit->attachment) . '"><i class="fa fa-chain"></i></a>' : ''); ?></td>
								<td><?= lang($deposit->paid_by); ?></td>
                                <td>
                                    <div class="text-center">
                                        <a href="<?= site_url('sale_orders/deposit_note/' . $deposit->id) ?>"
                                           data-toggle="modal" data-target="#myModal2"><i class="fa fa-file-text-o"></i></a>
                                        <?php if ($deposit->paid_by != 'gift_card') { ?>
                                            <a href="<?= site_url('sale_orders/edit_deposit/' . $deposit->id) ?>"
                                               data-toggle="modal" data-target="#myModal2"><i
                                                    class="fa fa-edit"></i></a>
                                            <a href="#" class="po"
                                               title="<b><?= $this->lang->line("delete_deposit") ?></b>"
                                               data-content="<p><?= lang('r_u_sure') ?></p><a class='btn btn-danger' id='<?= $deposit->id ?>' href='<?= site_url('sale_orders/delete_deposit/' . $deposit->id) ?>'><?= lang('i_m_sure') ?></a> <button class='btn po-close'><?= lang('no') ?></button>"
