    ...
    <div class="modal-content">
		...
		foreach($inv as $row){
			if($b==1){
				...
				$pid .='PurchaseID'.$row->id;
			}
			$total = $row->grand_total+$row->return_paid - ($row->paid+$row->discount+$row->return_total);
			$grand_total = $total;
			$total_amount += $total;
			$tbody .='<tr>	
        ...
        echo form_open_multipart("purchases/add_multi_payment/" . $pid, $attrib);
		?>
