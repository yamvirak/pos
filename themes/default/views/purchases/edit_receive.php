...
<script type="text/javascript">
    ...
    $(document).ready(function () {
		<?php if ($inv) { ?>
		localStorage.setItem('redate', '<?= date($dateFormats[($Settings->date_with_time == 0 ? 'php_sdate' : ($Settings->date_with_time == 0 ? 'php_sdate' : 'php_ldate'))], strtotime($inv->date))?>');
        localStorage.setItem('redateredateredate', '<?= date("d/m/Y H:i")?>');
        localStorage.setItem('rebiller', '<?=$inv->biller_id?>');
        localStorage.setItem('resupplier', '<?=$inv->supplier_id?>');
        localStorage.setItem('reref', '<?=$inv->re_reference_no?>');
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-plus"></i><?= lang('edit_receive'); ?></h2>
    </div>
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                echo form_open_multipart("purchases/edit_receive/".$id, $attrib);
				echo form_hidden('purchase_id', $inv->purchase_id);
				echo form_hidden('purchase_order_id', $inv->purchase_order_id);
                ?>
                <div class="row">
                    <div class="col-lg-12">
						...
						<div class="col-md-4 <?= ((!$Owner && !$Admin && !$GP['reference_no']) ? 'hidden' : '') ?>">
							<div class="form-group">
								...
								<?= form_input('reference_no', (isset($_POST['reference_no']) ? $_POST['reference_no'] : $inv->re_reference_no), 'class="form-control tip" id="reference_no"'); ?>
							</div>
						</div>
						<div class="col-md-4">
                            <div class="form-group">
                                <?= lang("si_reference_no", "posiref"); ?>
                                <?php echo form_input('si_reference_no', (isset($_POST['si_reference_no']) ? $_POST['si_reference_no'] : $inv->si_reference_no), 'class="form-control input-tip" id="posiref"'); ?>
                            </div>
                        </div>
						<div class="col-md-4">
                            <div class="form-group">
                                <?= lang("dn_reference", "dn_reference"); ?>
                                <?php echo form_input('dn_reference', (isset($_POST['dn_reference']) ? $_POST['dn_reference'] : $inv->dn_reference), 'class="form-control input-tip" id="dn_reference"'); ?>
                            </div>
                        </div>
						<?php if($this->config->item("concretes")){ ?>
							<div class="col-md-4">
								<div class="form-group">
									<?= lang("truck", "truck"); ?>
									<?php echo form_input('truck', (isset($_POST['truck']) ? $_POST['truck'] : $inv->truck), 'class="form-control input-tip" id="truck"'); ?>
								</div>
							</div>
						<?php } ?>
						...
						<div class="col-md-4" style="display:none">
							<div class="form-group">
								...
								<?php echo form_input('supplier', (isset($_POST['supplier']) ? $_POST['supplier'] : $supplier->name), 'class="form-control" id="resupplier" required="required" '); ?>
							</div>
						</div>
						...
						<div class="col-md-4">
							<div class="form-group">
								<?php
									...
									echo form_dropdown("receive_by", $us1, (isset($_POST['receive_by']) ? $_POST['receive_by'] : $inv->received_by)," class='form-control' id='receive_by' required='required' " );
								?>
							</div>
						</div>
                        ...
                        <div class="col-sm-12">
                            <div class="fprom-group"><?php echo form_submit('edit_receive', $this->lang->line("submit"), 'id="edit_receive" class="btn btn-primary" style="padding: 6px 15px; margin:15px 0;"'); ?>
