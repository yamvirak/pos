...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        <?php } if(isset($return) && $return){ ?>
			...
			localStorage.setItem('reref', '<?= $return->return_purchase_ref ?>');
		...
		$(document).on('change', '.punit', function () {
			...
			if((qty * unit_qty) > reitems[item_id].row.received) {
        ...
        $(document).on("focus", '.rquantity', function () {
            ...
            if (!is_numeric(new_qty) || ((new_qty * unit_qty) > reitems[item_id].row.received)) {
                $(this).val(old_row_qty);
                bootbox.alert('<?= lang('unexpected_value'); ?>');
                return false;
            }
            if((new_qty * unit_qty) > reitems[item_id].row.received) {
                bootbox.alert('<?= lang('unexpected_value'); ?>');
                $(this).val(old_row_qty);
                return false;
            }
    ...
    function loadItems() {
        ...
        if (localStorage.getItem('reitems')) {
            ...
            $.each(reitems, function () {
				...
				if(item.units !== false) {
					$.each(item.units, function () {
						if (item.row.return_unit == this.id){
							...
							s_unit_qty = this.operation_value;
				...
				var unit_cost = item.row.real_unit_cost * s_unit_qty;
				...
				if (site.settings.product_expiry == 1) {
                    tr_html += '<td class="text-center"><input type="hidden" value="'+item_expiry+'" name="expiry[]" />'+item_expiry+'</td>';
                }
				if (site.settings.product_serial == 1) {
					tr_html += '<td class="text-right"><input readonly class="form-control text-left rserial" name="serial_no[]" value="'+item.row.serial_no+'" id="serial_no_' + row_no + '"/></td>';
				}
                tr_html += '<td class="text-right"><input class="form-control input-sm text-right rcost" name="net_cost[]" type="hidden" id="cost_' + row_no + '" value="' + item_cost + '"><input class="rucost" name="unit_cost[]" type="hidden" value="' + unit_cost + '"><input class="realucost" name="real_unit_cost[]" type="hidden" value="' + item.row.real_unit_cost + '"><span class="text-right scost" id="scost_' + row_no + '">' + formatMoney(item.row.unit_cost) + '</span></td>';
                tr_html += '<td class="text-right"><span>'+item.row.qty_with_unit+'</span></td>';
                if (po_edit) {
                    tr_html += '<td class="text-right"><span>'+item.row.rec_with_unit+'</span></td>';
                }
                ...
                tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney(((parseFloat(unit_cost) + parseFloat(pr_tax_val)) * parseFloat(item_qty)) - (item_discount * item_qty)) + '</span></td>';
                ...
                total += formatDecimal(((parseFloat(unit_cost) + parseFloat(pr_tax_val)) * parseFloat(item_qty)) - (item_discount * item_qty));
            ...
            <div class="col-lg-12">
                ...
                <div class="row">
                    <div class="col-lg-12">
						...
						<input type="hidden" name="discount_rate" value="<?= (100*$inv->order_discount)/$inv->total ?>" id="discount_rate" required="required"/>
