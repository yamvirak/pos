...
<script>
    $(document).ready(function () {
		...
		<?php if($this->session->userdata('remove_rels')) { ?>
			...
			<?php $this->cus->unset_data('remove_rels'); ?>
		<?php } ?>
        ...
        oTable = $('#EXPData1').dataTable({
            ...
            'sAjaxSource': '<?= site_url('purchases/getReceives/'.($biller ? $biller->id : '')); ?>',
            ...
            "aoColumns": [{
                "mRender": checkbox
            }, {"mRender": fld}, null, null, null, null, null,{"mRender": row_status}, {
                "bSortable": false,
                "mRender": attachment
            }, {"bSortable": false}],
			...
			{column_number: 3, filter_default_label: "[<?=lang('pu_reference');?>]", filter_type: "text", data: []},
			...
			{column_number: 6, filter_default_label: "[<?=lang('received_by');?>]", filter_type: "text", data: []}
        ], "footer");
...
<?php if ($Owner) {
    echo form_open('purchases/receive_actions', 'id="action-form"');
} ?>
<div class="box">
    <div class="box-header">
		<h2 class="blue"><i class="fa-fw fa fa-dollar"></i><?= lang('receives').' ('.($biller ? $biller->name : lang('all_billers')).')'; ?></h2>	
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                        <li>
							<a href="<?=site_url('purchases/add_receive')?>">
								<i class="fa fa-plus-circle"></i> <?=lang('add_receive_item')?>
							</a>
						</li>
						<?php if($this->config->item("po_receive_item") && $Owner || $Admin || $GP['purchases-add']){ ?>
							<li><a href="#" id="add_purchase" data-action="add_purchase"><i class="fa fa-plus-circle"></i> <?= lang('add_purchase') ?></a></li>
						<?php } ?>
						<li>
                        ...
                        <li class="divider"></li>
                        <li>
                            <a href="#" class="bpo" title="<b><?= $this->lang->line("delete_receives") ?></b>" 
                                data-content="<p><?= lang('r_u_sure') ?></p><button type='button' class='btn btn-danger' id='delete' data-action='delete'><?= lang('i_m_sure') ?></a> <button class='btn bpo-close'><?= lang('no') ?></button>" 
                                data-html="true" data-placement="left">
                                <i class="fa fa-trash-o"></i> <?= lang('delete_receives') ?>
                            </a>
                        </li>
                    </ul>
                </li>
				<?php if (!empty($billers) && $this->config->item('one_biller')==false) { ?>
					<li class="dropdown">
						...
						<ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
							<li><a href="<?= site_url('purchases/receives') ?>"><i class="fa fa-industry"></i> <?= lang('all_billers') ?></a></li>
							<li class="divider"></li>
							<?php
							foreach ($billers as $biller) {
								echo '<li><a href="' . site_url('purchases/receives/'.$biller->id) . '"><i class="fa fa-home"></i>' . $biller->name . '</a></li>';
							}
							?>
						</ul>
					</liv>
                ...
                <div class="table-responsive">
                    <table id="EXPData1" cellpadding="0" cellspacing="0" border="0"
                        ...
                        <tr class="active">
                            ...
                            <th width="100"><?= lang("reference"); ?></th>
							<th width="100"><?= lang("pu_reference"); ?></th>
							<th width="150"><?= lang("supplier"); ?></th>
							<th width="150"><?= lang("note"); ?></th>
                            <th width="100"><?= lang("received_by"); ?></th>
