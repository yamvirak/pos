    ...
    <div class="modal-content">
		<div class="modal-header">
			...
			<?php if($pos_settings->pos_favorite_items==1){ ?>
				...
				<button type="button" class="btn btn-xs btn-default no-print pull-right d-rate" style="margin-right:5px;" onclick="rate(<?= $product->id; ?>);">
        ...
        <div class="modal-body">
			<div class="alerts-con-modal"></div>
            <div class="row">
                <div class="col-xs-5">
						...
						<table class="table table-borderless table-striped dfTable table-right-left">
							<tbody>
								...
								<tr>
									...
									<td><?= $this->cus->formatQuantity($product->alert_quantity); ?></td>
								</tr>
								<?php } ?>
								<?php if ($variants) { ?>
								<tr>
				...
				<div class="col-xs-12">
					<div class="row">
						<div class="col-xs-5">
							...
							<?php if (($Owner || $Admin || $this->session->userdata('show_price')) && $price_groups) { ?>
							<h3 class="bold"><?= lang('price_groups') ?></h3>		
							...
							<div class="table-responsive">
								<table
									...
									<tbody>
										<?php foreach ($price_groups as $price_group) {
											if ($price_group->price > 0) {
												echo '<tr><td>'.$price_group->name.' </td><td class="text-right"><strong>' . $this->cus->formatMoney($price_group->price) . '</strong></td></tr>';
											}
										} ?>
									</tbody>
								</table>
							...
							<?php if ((!$Supplier || !$Customer) && !empty($warehouses) && ($product->type == 'standard' || $product->type == 'raw_material' || $product->type == 'asset')) { ?>
									...
									<tbody>
										<?php foreach ($warehouses as $warehouse) {
											if ($warehouse->quantity != 0) {
												echo '<tr><td>' . $warehouse->name . ' (' . $warehouse->code . ')</td><td><strong>' . $this->cus->convertQty($product->id,$warehouse->quantity) . '</strong>' . ($warehouse->rack ? ' (' . $warehouse->rack . ')' : '') . '</td></tr>';
											}
										} ?>
									</tbody>
								</table>
						...
						<div class="col-xs-7">
										...
										foreach ($options as $option) {
											if ($option->wh_qty != 0) {
												...
												if ($Owner || $Admin && (!$Customer || $this->session->userdata('show_cost'))) {
													echo '<td class="text-right">' . $this->cus->formatMoney($option->price) . '</td>';
												}
												echo '</tr>';
											}
							...
							<?php if(isset($expiries) && $expiries){ ?>
								<h3 class="bold"><?= lang('expiry_quantity') ?></h3>
								<div class="table-responsive">
									<table
										class="table table-bordered table-striped table-condensed dfTable two-columns">
										<thead>
										...
										<tbody>
											<?php
												foreach($expiries as $expiry){
													if($expiry->quantity && $expiry->quantity != 0){
														echo '<tr>
															<td>'.$expiry->warehouse_name.'</td>
															<td class="center">'.$this->cus->hrsd($expiry->expiry).'</td>
															<td>'.$this->cus->convertQty($expiry->product_id, $expiry->quantity).'</td>
														</tr>';
													}
