...
<script>
    $(document).ready(function () {
        oTable = $('#convertData').dataTable({
            "aaSorting": [[0, "desc"]],
            "aLengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "<?= lang('all') ?>"]],
            "iDisplayLength": <?= $Settings->rows_per_page ?>,
            'bProcessing': true, 'bServerSide': true,
            'sAjaxSource': '<?= site_url('converts/getConvertItems/'.($warehouse ? $warehouse->id : 0).'/'.($biller ? $biller->id : '')); ?>',
			...
			{column_number: 3, filter_default_label: "[<?=lang('bom');?>]", filter_type: "text", data: []},
        ...
        if (localStorage.getItem('remove_cvls')) {
            ...
            localStorage.removeItem('remove_cvls');
        }
...
<div class="box">
    <div class="box-header">
        ...
        <div class="sub_menu">
        	<a class="btn btn-success btn-block box_sub_menu" href="<?= site_url('converts/add') ?>">
               	<i class="fa fa-plus-circle"></i> <?= lang('add_converts') ?>
            </a>
        </div>
        <div class="sub_menu">
        ...
        <div class="sub_menu">
        	<a href="#" class="bpo box_sub_menu btn btn-danger btn-block" title="<b><?= $this->lang->line("delete_converts") ?></b>"
                 data-content="<p><?= lang('r_u_sure') ?></p><button type='button' class='btn btn-danger' id='delete' data-action='delete'><?= lang('i_m_sure') ?></a> <button class='btn bpo-close'><?= lang('no') ?></button>"
                 data-html="true" data-placement="left">
                <i class="fa fa-trash-o"></i> <?= lang('delete_converts') ?>
            </a>
        </div>
        ...
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
                    <h2 class="blue"><i class="icon fa fa-list-ol"></i><?= lang('converts'); ?></h2>
                </li>
				<?php if (!empty($warehouses) && $this->config->item('one_warehouse')==false)  { ?>
					<li class="dropdown">
						<a data-toggle="dropdown" class="dropdown-toggle" href="#"><i class="icon fa fa-building-o tip" data-placement="left" title="<?= lang("warehouses") ?>"></i></a>
						<ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
							<li><a href="<?= site_url('products/converts') ?>"><i class="fa fa-building-o"></i> <?= lang('all_warehouses') ?></a></li>
							<li class="divider"></li>
							<?php
							foreach ($warehouses as $warehouse) {
								echo '<li><a href="' . site_url('products/converts/' . $warehouse->id) . '"><i class="fa fa-building"></i>' . $warehouse->name . '</a></li>';
							}
							?>
						</ul>
					</li>
				...
				<?php if (!empty($billers) && $this->config->item('one_biller')==false) { ?>
					<li class="dropdown">
						...
						<ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
							<li><a href="<?= site_url('products/converts') ?>"><i class="fa fa-industry"></i> <?= lang('all_billers') ?></a></li>
							<li class="divider"></li>
							<?php
							foreach ($billers as $biller) {
								echo '<li><a href="' . site_url('products/converts/null/'.$biller->id) . '"><i class="fa fa-home"></i>' . $biller->name . '</a></li>';
							}
							?>
						</ul>
					</li>
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="table-responsive">
                    <table id="convertData" class="table table-bordered table-condensed table-hover table-striped">
                        <thead>
							<tr>
