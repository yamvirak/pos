...
<script type="text/javascript">
	<?php if ($convert) { ?>
        localStorage.setItem('cvdate', '<?= $this->cus->hrld($convert->date) ?>');
        localStorage.setItem('cvref', '<?= $convert->reference_no ?>');
        localStorage.setItem('cvbiller', '<?= $convert->biller_id ?>');
		localStorage.setItem('cvwarehouse', '<?= $convert->warehouse_id ?>');
        localStorage.setItem('cvbom', '<?= $convert->bom_id ?>');
		localStorage.setItem('cvquantity', '<?= $convert->quantity ?>');
        localStorage.setItem('cvnote', '<?= str_replace(array("\r", "\n", "'"), "", $this->cus->decode_html($convert->note)); ?>');
    <?php } ?>
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <?php
					...
					echo form_open_multipart("converts/edit_convert/".$convert->id, $attrib);
                ?>
					<?php if ($Owner || $Admin || $GP['products-converts-date']) { ?>
						<div class="col-md-4">
					...
					<div class="col-md-12">
						<div class="fprom-group"><?php echo form_submit('edit_convert', lang("submit"), 'id="edit_convert" class="btn btn-primary" style="padding: 6px 15px; margin:15px 0;"'); ?>
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		function loadItems(){
			$('#edit_convert, #edit_convert').attr('disabled', false);
			$('#rawTable tbody .product_id').each(function(index) {
				...
				if(quantity > qoh) { 
					row.addClass('danger');
					if(site.settings.overselling != 1) { $('#edit_convert, #edit_convert').attr('disabled', true); }
				}
			});
		}
		...
		function biller(){
			...
			var project = "<?= $convert->project_id ?>";
			$.ajax({
