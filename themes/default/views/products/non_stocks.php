...
<script>
    ...
    $(document).ready(function () {
        oTable = $('#PRData').dataTable({
            ...
            'sAjaxSource': '<?= site_url('products/getProductsNonStock'.($warehouse_id ? '/'.$warehouse_id : '').($supplier ? '?supplier='.$supplier->id : '')) ?>',
...
<div class="box">
    <div class="box-header">
        ...
        <div class="sub_menu">
            <a href="<?= site_url('products/add_non_stock') ?>" class="btn btn-success btn-block box_sub_menu" 
                tabindex="-1">
                <i class="fa fa-plus-circle"></i> <?= lang('Add_Non_Stock') ?>
            </a>
        </div>
        <div class="sub_menu">
            <?php if(!$warehouse_id) { ?>
