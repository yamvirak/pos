    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                echo form_open_multipart("products/add_non_stock", $attrib)
                ?>
                ...
                <div class="col-md-5">
					...
					<?php if($this->config->item("concretes")){ ?>
						<div style="display:none;" class="form-group bom">
                    ...
                    <div class="form-group standard">
                        ...
                        <?= form_input('cost', (isset($_POST['cost']) ? $_POST['cost'] : ($product ? $this->cus->formatDecimal($product->cost) : 0)), 'class="form-control tip" id="cost" required="required"') ?>
                    </div>
					...
					<?php if($this->config->item('product_currency')==true){?>
						<div class="form-group">
							...
							?>
							<?= form_dropdown('currency_code', $copts, $Settings->default_currency, 'class="form-control" id="currency_code" style="width:100%;" required="required" '); ?>
						</div>
					<?php } ?>
					
                    <div class="form-group all standard hide_raw services div_price">
                        <?= lang("product_price", "price") ?>
                        <?= form_input('price', (isset($_POST['price']) ? $_POST['price'] : ($product ? $this->cus->formatDecimal($product->price) : '')), 'class="form-control tip" id="price" required="required"') ?>
                    </div>
					
					<?php if($this->Settings->product_additional==1){ ?>
						<div class="form-group standard">
					...
					<?php } if($this->Settings->product_serial==1){ ?>
    					<div class="form-group standard">
                            <input type="checkbox" class="checkbox" value="1" name="seperate_qty" id="seperate_qty" />
				   ...
				   <?php } ?>
                    ...
                    <?php if ($Settings->tax1) { ?>
                     ...
                     <?php } if($pos_settings->table_enable == 1){ ?>
						<div class="form-group">
							<input name="adjustment_qty" type="checkbox" class="checkbox" id="adjustment_qty" value="1" <?= isset($_POST['adjustment_qty']) ? 'checked="checked"' : '' ?>/>
                    ...
                    <div class="form-group standard">
                        ...
                        <div
                            class="input-group"> <?= form_input('alert_quantity', (isset($_POST['alert_quantity']) ? $_POST['alert_quantity'] : ($product ? $this->cus->formatQuantity($product->alert_quantity) : '')), 'class="form-control tip" id="alert_quantity"') ?>
										...
										<?php
											foreach($product_unit as $productunit_row){
												echo'<tr>
														...
														<td>
															<input '.($productunit_row->id==$product->unit?'readonly':'').' type="text" name="product_unit_price[]" class="form-control" placeholder="'.lang('price').'" />
														</td>
													</tr>';
											}
										?>
                                ...
                                <table id="attrTable" class="table table-bordered table-condensed table-striped"
                                    ...
                                    if ($this->input->post('attributes')) {
                                        ...
                                        foreach ($product_options as $option) {
                                            echo '<tr class="attr"><td><input type="hidden" name="attr_name[]" value="' . $option->name . '"><span>' . $option->name . '</span></td><td class="code text-center"><input type="hidden" name="attr_warehouse[]" value="' . $option->warehouse_id . '"><input type="hidden" name="attr_wh_name[]" value="' . $option->wh_name . '"><span>' . $option->wh_name . '</span></td><td class="quantity text-center"><input type="hidden" name="attr_quantity[]" value="' . $this->cus->formatQuantity($option->wh_qty) . '"><span>' . $this->cus->formatQuantity($option->wh_qty) . '</span></td><td class="price text-right"><input type="hidden" name="attr_price[]" value="' . $this->cus->formatMoney($option->price) . '"><span>' . $this->cus->formatMoney($option->price) . '</span></span></td><td class="text-center"><i class="fa fa-times delAttr"></i></td></tr>';
                                        }
                                    }
                                    ?></tbody>
                                </table>
				...
				<?php if($Settings->accounting == 1){ ?>
					...
					<div style="display:none" class="form-group services">
    ...
    $(document).ready(function () {
		...
		function add_bom_product_item(item) {
            ...
            $.each(items, function () {
                ...
                tr_html += '<td><input class="form-control text-center" type="text" name="bom_type[]"/></td>';
		...
		$(document).on("focus", '.bom_item_quantity', function () {
			old_quantity = $(this).val();
		}).on("change", '.bom_item_quantity', function () {
			var row = $(this).closest('tr');
			...
			item_id = row.attr('data-item-id');
			items[item_id].qty = new_quantity;
		});
...
//});
    ...
    $(document).ready(function () {
        ...
        $('#supplier_price').val("<?= $product->supplier1price == 0 ? '' : $this->cus->formatDecimal($product->supplier1price); ?>");
        <?php } ?>
        <?php if ($product->supplier2) { ?>
        $('#addSupplier').click();
        select_supplier('supplier_2', "<?= $product->supplier2; ?>");
        $('#supplier_2_price').val("<?= $product->supplier2price == 0 ? '' : $this->cus->formatDecimal($product->supplier2price); ?>");
        <?php } ?>
        <?php if ($product->supplier3) { ?>
        $('#addSupplier').click();
        select_supplier('supplier_3', "<?= $product->supplier3; ?>");
        $('#supplier_3_price').val("<?= $product->supplier3price == 0 ? '' : $this->cus->formatDecimal($product->supplier3price); ?>");
        <?php } ?>
        <?php if ($product->supplier4) { ?>
        $('#addSupplier').click();
        select_supplier('supplier_4', "<?= $product->supplier4; ?>");
        $('#supplier_4_price').val("<?= $product->supplier4price == 0 ? '' : $this->cus->formatDecimal($product->supplier4price); ?>");
        <?php } ?>
        <?php if ($product->supplier5) { ?>
        $('#addSupplier').click();
        select_supplier('supplier_5', "<?= $product->supplier5; ?>");
        $('#supplier_5_price').val("<?= $product->supplier5price == 0 ? '' : $this->cus->formatDecimal($product->supplier5price); ?>");
        <?php } ?>
