...
<div class="modal-dialog modal-lg">
	...
	<table>
		<thead>
					...
					<table style="margin-top: 8px;">
						<tr>
							...
							<td class="text_center" style="width:20%">
								<?= $this->cus->qrcode('link', urlencode(site_url('sales/deposit_note/' . $deposit->id)), 2); ?>
							</td>
						</tr>
					</table>
				</th>
			...
			<tr>
				<td class="text_center"><span style="font-size:<?= $font_size+5 ?>px"><b><?= lang('inv_receipt_voucher') ?></b></span></td>
			</tr>

		

			<?php
				if ($deposit->paid_by == 'gift_card' || $deposit->paid_by == 'CC' || $f->paid_by == 'ppp' || $deposit->paid_by == 'stripe' || $deposit->paid_by == 'authorize') {
		...
		<tbody>
			...
			<table border="0" style="padding:5px;">
				<tr>
					<td colspan="3"></td>
					<td>លេខ <br> NO.</td>
					<td><div class="bold"><?= $deposit->reference_no ?></div></td>
				</tr>
				...
				<tr>
					...
					<td style="width:30%;">
						<?= $customer->company?>
							<?php 
                                if($customer->khmer_name_v2 == NULL){
                                    echo '';
                                }else{
                                    echo 'និង&nbsp;'.$customer->khmer_name_v2;
                                }
                         ?>
						<br>
					...
					<td><div class="name_receive"><?= $this->cus->formatMoney($deposit->amount) ?></div></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td style="width:30%;"><?=convert_number_to_words($deposit->amount).'ដុល្លារសហរដ្ឋអាមេរិកគត់';?> <br>
						<div style="border-bottom:1px solid black;width: 100%;"></div>
					</td>
					<td width="50px;">&nbsp; </td>
					<td>ទូទាត់ដោយ<br> Paying By</td>
					<td><div class="name_receive"><?= $deposit->cash_account.$deposit_info ?></div></td>
				...
				<tr>
					<td width="130">អត្ថន័យ <br> Being In deposit</td>
					<td style="width:30%;"> <?= html_entity_decode($deposit->note); ?><br>
						<div style="border-bottom:1px solid black;width: 100%;"></div>
					</td>
					<td width="50px;">&nbsp; </td>
					<td>បរិយាយ<br> Description</td>
					<td><div class="name_receive"><?= $inv->description ?></div></td>
				</tr>
				<tr>
					<td><div style="margin: -6px;">&nbsp;</div></td>
				</tr>
				<tr>
					...
					<td><div class="name_receive"><?= $this->cus->hrsd($deposit->date) ?></div></td>
				</tr>
				<tr>
					<td><div style="margin: -6px;">&nbsp;</div></td>
				</tr>
				...
				<td>
					<table class="table_item">
						...
						<tbody id="tfooter" class="hidden">
							...
							<?php if($deposit->note){ ?>
								<tr>
									<td style="border:none !important" colspan="5"><b><?= lang('note') ?> : </b><?= html_entity_decode($deposit->note); ?></td>
								</tr>
							<?php } ?>
						</tbody>
					</table>
				...
				<td>
					 ...
					 <table style="margin-top:5px; margin-bottom:<?= $margin_signature -60;?>px;">
                                <thead class="footer_items">
                                    <th class="text_cen con_content" style="width: 50%;"><?= lang(" អតិថិជន/Customer");?></th>
                                ...
                                <thead class="footer_item_footers">
                                    <th class="footer_item_footers">
		                                <div class="footer_name">ឈ្មោះ <?= $customer->company?></div>
		...
		<tbody>
			...
			<table border="0" style="padding:5px;">
				...
				<tr>
					...
					<td width="50px;">&nbsp; </td>
					<td>ទូទាត់ដោយ<br> Paying By</td>
					<td><div class="name_receive"><?= lang($deposit->cash_account).' '.$deposit_info ?></div></td>
				...
				<tr>
					<td width="130">អត្ថន័យ <br> Being In Payment</td>
					<td style="width:30%;"> <?= html_entity_decode($deposit->note); ?><br>
						<div style="border-bottom:1px solid black;width: 100%;"></div>
					</td>
					<td width="50px;">&nbsp; </td>
	...
	<div class="buttons" style="margin-top:20px; margin-bottom:20px">
		<div class="btn-group btn-group-justified">
			...
			<?php if($inv_payments && $inv_payment->installment_item_id <= 0){ ?>
						<div class="btn-group">
							<a data-toggle="modal" data-target="#myModal2" class="tip btn btn-warning" href="<?= site_url("sales/edit_payment/".$deposit->id) ?>" title="<?= lang('edit') ?>">
