<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-th-large"></i><?= lang('cash_book'); ?>
			<?php
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    ...
                    <?php echo form_open("accountings/cash_books"); ?>
                ...
                <div class="table-responsive">
                    <table cellpadding="0" cellspacing="0" style="white-space:nowrap;" border="1" class="table table-bordered table-hover table-striped table-condensed accountings-table dataTable">
						...
						<tbody>
						<?php
							$getAccWithBegins = $this->accountings_model->getAccWithBegin(1);
							$html = '';
							if($getAccWithBegins){
								...
								foreach($getAccWithBegins as $getAccWithBegin){
									if($getAccWithBegin->amountBegin > 0){
										$begin_balance = $this->cus->formatMoney($getAccWithBegin->amountBegin);
									}else{
										$begin_balance = '('.$this->cus->formatMoney(abs($getAccWithBegin->amountBegin)).')';
									}
									$html .='<tr style="font-weight:bold; color:#39c65c">
												<td colspan="12">'.$getAccWithBegin->account.' - '.$getAccWithBegin->name.'</td>
												<td style="text-align:right">'.$begin_balance.'</td>
											</tr>';
											
									$accDetails = $accTrans[$getAccWithBegin->account];
									if($accDetails){
										...
										foreach($accDetails as $accDetail){
											...
											if($balance > 0){
												$balance_show = $this->cus->formatMoney($balance);
											}else{
											...
											$html .='<tr>
														...
														<td>'.$accDetail["product_name"].'</td>
														<td>'.$accDetail["description"].'</td>
