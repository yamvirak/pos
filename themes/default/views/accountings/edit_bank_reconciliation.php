...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-plus"></i><?= lang('edit_bank_reconciliation'); ?></h2>
    </div>
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                echo form_open_multipart("accountings/edit_bank_reconciliation/".$bank_reconciliation->id, $attrib);
                ?>
                <div class="row">
                    <div class="col-lg-12">
                        <?php if ($Owner || $Admin || $GP['accountings-bank_reconciliation-date']) { ?>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <?= lang("date", "brdate"); ?>
                                    <?php echo form_input('date', (isset($_POST['date']) ? $_POST['date'] : $this->cus->hrld($bank_reconciliation->date)), 'class="form-control input-tip datetime" id="brdate" required="required"'); ?>
                                </div>
                            </div>
                        <?php } ?>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <?php echo form_input('reference_no', (isset($_POST['reference_no']) ? $_POST['reference_no'] : $bank_reconciliation->reference), 'class="form-control input-tip" id="brref"'); ?>
                            </div>
                        </div>
						<?php if ($Owner || $Admin || !$this->session->userdata('biller_id')) { ?>
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    echo form_dropdown('biller', $bl, (isset($_POST['biller']) ? $_POST['biller'] : $bank_reconciliation->biller_id), 'id="slbiller" data-placeholder="' . lang("select") . ' ' . lang("biller") . '" required="required" class="form-control input-tip select" style="width:100%;"');
                                    ?>
                                </div>
                            </div>
                        <?php } else {
								...
								<div class="col-md-4">
									<div class="form-group">
										...
										<div class="no-project">
											...
											echo form_dropdown('project', $pj, (isset($_POST['project']) ? $_POST['project'] : $bank_reconciliation->project_id), 'id="brproject" class="form-control input-tip select" data-placeholder="' . lang("select") . ' ' . lang("project") . '" style="width:100%;" ');
											?>
										</div>
									</div>
								</div>
						...
						<div class="col-md-4">
							<div class="form-group">
								...
								<?php echo form_input('statement_date', (isset($_POST['statement_date']) ? $_POST['statement_date'] : $this->cus->hrsd($bank_reconciliation->statement_date)), 'class="form-control input-tip date" id="statement_date" required="required"'); ?>
							</div>
						</div>
						<div class="col-md-4">
							<div class="form-group">
								<?= lang("beginning_balance", "beginning_balance"); ?>
								<?php echo form_input('beginning_balance', (isset($_POST['beginning_balance']) ? $_POST['beginning_balance'] : $this->cus->formatDecimal($bank_reconciliation->beginning_balance)), 'class="form-control input-tip text-right" id="beginning_balance" readonly="true"'); ?>
							</div>
						</div>
						<div class="col-md-4">
							<div class="form-group">
								<?= lang("ending_balance", "ending_balance"); ?>
								<?php echo form_input('ending_balance', (isset($_POST['ending_balance']) ? $_POST['ending_balance'] : $this->cus->formatDecimal($bank_reconciliation->ending_balance)), 'class="form-control input-tip text-right" id="ending_balance" required="required"'); ?>
							</div>
						</div>
						<div class="col-md-12">
							...
							<table id="brTable" class="table items table-striped table-bordered table-condensed table-hover">
								...
								<tr>
									...
									<td><input class="form-control text-right" value="<?= $this->cus->formatDecimal($bank_reconciliation->service_charge) ?>" type="text" name="service_charge" id="service_charge"/></td>
								</tr>
								<tr>
									...
									<td><input class="form-control text-right" value="<?= $this->cus->formatDecimal($bank_reconciliation->interest_earned) ?>" type="text" name="interest_earned" id="interest_earned"/></td>
								</tr>
							</table>
						</div>
                                    ...
                                    <table id="brTable" class="table items table-striped table-bordered table-condensed table-hover">
                                        ...
                                        <tbody id="brBody">
											<?php
												$tbody = '';
												if($bank_recon_items){
													foreach($bank_recon_items as $bank_recon_item){
														$tbody .= '<tr>';
														$tbody .= '<td><input name="cleared[]" class="cleared" type="hidden" value="'.$bank_recon_item->cleared.'"/> <input class="checkbox multi-select input-xs check_clear" '.($bank_recon_item->cleared == 1 ? 'checked' : '').' value="'.$bank_recon_item->id.'" type="checkbox" name="val[]""/></td>';
														$tbody .= '<td><input type="hidden" value="'.$bank_recon_item->transaction_id.'" name="transaction_id[]" class="transaction_id"/><input type="hidden" value="'.$bank_recon_item->transaction.'" name="transaction[]" class="transaction"/>'.$bank_recon_item->transaction.'</td>';
														$tbody .= '<td class="text-center"><input type="hidden" value="'.$bank_recon_item->transaction_date.'" name="transaction_date[]" class="transaction_date"/>'.$this->cus->hrsd($bank_recon_item->transaction_date).'</td>';
														$tbody .= '<td><input type="hidden" value="'.$bank_recon_item->reference.'" name="reference[]" class="reference"/>'.$bank_recon_item->reference.'</td>';
														$tbody .= '<td><input type="hidden" value="'.$bank_recon_item->narrative.'" name="narrative[]" class="narrative"/>'.$bank_recon_item->narrative.'</td>';
														$tbody .= '<td><input type="hidden" value="'.$bank_recon_item->description.'" name="description[]" class="description"/>'.$bank_recon_item->description.'</td>';
														if($bank_recon_item->amount > 0){
															$tbody .= '<td class="text-right"><input type="hidden" value="'.$bank_recon_item->amount.'" name="amount[]" class="amount"/>'.$this->cus->formatMoney($bank_recon_item->amount).'</td>';
															$tbody .= '<td></td>';
														}else {
															$tbody .= '<td></td>';
															$tbody .= '<td class="text-right"><input type="hidden" value="'.$bank_recon_item->amount.'" name="amount[]" class="amount"/>'.$this->cus->formatMoney(abs($bank_recon_item->amount)).'</td>';
														}
														$tbody .= '</tr>';
													}
												}
                        ...
                        <div class="col-md-12">
                            <div
                                class="fprom-group"><?php echo form_submit('edit_bank_reconciliation', lang("submit"), 'id="submit_journal" class="btn btn-primary" style="padding: 6px 15px; margin:15px 0;"'); ?>
...
<script type="text/javascript">
	$(function(){
		...
		function biller(){
			...
			var project = "<?= $bank_reconciliation->project_id ?>";
			$.ajax({
