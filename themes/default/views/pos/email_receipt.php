                    ...
                    <tr>
                        <td>
                            ...
                            <div style="text-align:left;">
                                ...
                                <?= $this->cus->qrcode('link', urlencode(site_url('pos/view/' . $inv->id)), 2); ?>
                            </div>
                            <div style="clear:both;height:15px;"></div>
                                <div id="receiptData" style="border:1px solid #DDD; padding:10px; text-align:center;">
                                    ...
                                    echo lang("date") . ": " . $this->cus->hrld($inv->date) . "</p>";
                                    ?>
                                    ...
                                    <table width="100%" style="margin:15px 0;">
                                        ...
                                        foreach ($rows as $row) {
                                            ...
                                            echo '<tr><td style="border-bottom:1px solid #DDD;">' . $this->cus->formatQuantity($row->quantity) . ' x ';
                                            if ($row->item_discount != 0) {
                                                echo '<del>' . $this->cus->formatMoney($row->net_unit_price + ($row->item_discount / $row->quantity) + ($row->item_tax / $row->quantity)) . '</del> ';
                                            }
                                            echo $this->cus->formatMoney($row->net_unit_price + ($row->item_tax / $row->quantity)).' ('.$this->cus->formatMoney($row->net_unit_price).' + '.$this->cus->formatMoney($row->item_tax / $row->quantity) . ')</td><td style="text-align:right;border-bottom:1px solid #DDD;">' . $this->cus->formatMoney($row->subtotal) . '</td></tr>';
                                            $r++;
                                        }
                                        ?>
                                        </tbody>
                                        ...
                                        <tr>
                                            ...
                                            <th style="text-align:right;border-bottom:1px solid #DDD;"><?= $this->cus->formatMoney($inv->total); ?></th>
                                        </tr>
                                        <?php
                                        if ($inv->order_tax != 0 && $Settings->tax2) {
                                            echo '<tr><th style="text-align:left;border-bottom:1px solid #DDD;">' . lang("tax") . '</th><th style="text-align:right;border-bottom:1px solid #DDD;">' . $this->cus->formatMoney($inv->order_tax) . '</th></tr>';
                                        }
                                        if ($inv->total_discount != 0) {
                                            echo '<tr><th style="text-align:left;border-bottom:1px solid #DDD;">' . lang("discount") . '</th><th style="text-align:right;border-bottom:1px solid #DDD;">' . $this->cus->formatMoney($inv->total_discount) . '</th></tr>';
                                        }
                                        ?>
                                        <tr>
                                            <th style="text-align:left;border-bottom:1px solid #DDD;"><?= lang("grand_total"); ?></th>
                                            <th style="text-align:right;border-bottom:1px solid #DDD;"><?= $this->cus->formatMoney($inv->grand_total); ?></th>
                                        </tr>
                                        <?php if ($inv->rounding) { ?>
                                            <tr>
                                                <th style="text-align:left;border-bottom:1px solid #DDD;"><?=lang("rounding");?></th>
                                                <th style="text-align:right;border-bottom:1px solid #DDD;"><?= $this->cus->formatMoney($inv->rounding);?></th>
                                            </tr>
                                            <tr>
                                                <th style="text-align:left;border-bottom:1px solid #DDD;"><?=lang("grand_total");?></th>
                                                <th style="text-align:right;border-bottom:1px solid #DDD;"><?=$this->cus->formatMoney(($inv->grand_total + $inv->rounding));?></th>
                                            </tr>
                                        <?php } else { ?>
                                            <tr>
                                                <th style="text-align:left;border-bottom:1px solid #DDD;"><?=lang("grand_total");?></th>
                                        ...
                                        <?php } if ($inv->paid < $inv->grand_total) { ?>
                                            <tr>
                                                ...
                                                <th style="text-align:right;border-bottom:1px solid #DDD;"><?=$this->cus->formatMoney($inv->paid);?></th>
                                            </tr>
                                            <tr>
                                                <th style="text-align:left;border-bottom:1px solid #DDD;"><?=lang("due_amount");?></th>
                                                <th style="text-align:right;border-bottom:1px solid #DDD;"><?=$this->cus->formatMoney((($inv->grand_total + $inv->rounding)) - ($inv->paid));?></th>
                                            </tr>
                                        <?php } ?>
                                    ...
                                    if ($payments) {
                                        ...
                                        foreach ($payments as $payment) {
                                            ...
                                            if ($payment->paid_by == 'cash' && $payment->amount) {
                                                ...
                                                echo '<td>' . lang("change") . ': ' . $this->cus->formatMoney($payment->amount - $inv->total) . '</td>';
                                            }
                                    ...
                                    <p class="text-center">
                                        <?= $this->cus->decode_html($biller->invoice_footer); ?>
                                    </p>
                                    <div style="clear:both;"></div>
                                </div>
