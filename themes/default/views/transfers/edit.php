...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        <?php if ($transfer) { ?>
        localStorage.setItem('todate', '<?= date($dateFormats[($Settings->date_with_time == 0 ? 'php_sdate' : 'php_ldate')], strtotime($transfer->date)) ?>');
		...
		localStorage.setItem('fr_biller', '<?= $transfer->biller_id ?>');
		localStorage.setItem('to_biller', '<?= $transfer->to_biller_id ?>');
        localStorage.setItem('tostatus', '<?= $transfer->status ?>');
        localStorage.setItem('tonote', '<?= $this->cus->decode_html($transfer->note); ?>');
                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="col-md-4 <?= ((!$Owner && !$Admin && !$GP['reference_no']) ? 'hidden' : '') ?>">
                            <div class="form-group">
                                ...
                                <?php echo form_input('reference_no', (isset($_POST['reference_no']) ? $_POST['reference_no'] : $transfer->transfer_no), 'class="form-control input-tip" id="ref" '); ?>
                            </div>
                        </div>
                                ...
                                <div class="panel-body" style="padding: 5px;">
                                    ...
                                    <div class="col-md-4">
										<div class="form-group">
											...
											echo form_dropdown('to_warehouse', $whf, (isset($_POST['to_warehouse']) ? $_POST['to_warehouse'] : $Settings->default_warehouse), 'id="to_warehouse" class="form-control input-tip select" data-placeholder="' . $this->lang->line("select") . ' ' . $this->lang->line("to_warehouse") . '" required="required" style="width:100%;" ');
											?>
										</div>
									</div>
                                </div>
...
<script type="text/javascript">
    $(document).ready(function () {
		...
		$('#fr_biller').change(function (e) {
			biller($(this).val());
		});
		function biller(biller){
			var project = '<?= $transfer->from_project ?>';
			$.ajax({
		...
		$('#to_biller').change(function (e) {
			to_biller($(this).val());
		});
		
		$("#to_biller").change(to_biller); to_biller();
		to_biller(localStorage.getItem('to_biller'));
		function to_biller(to_biller){
			var to_project = '<?= $transfer->to_project ?>';
			$.ajax({
