...
<div class="modal-dialog">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title" id="myModalLabel"><?php echo lang('add_locations'); ?></h4>
        </div>
        <?php $attrib = array('data-toggle' => 'validator', 'role' => 'form');
        echo form_open_multipart("system_settings/add_locations", $attrib); ?>
            ...
            <p><?= lang('enter_info'); ?></p>
		            	...
		            	<div class="form-group">
							...
							foreach($types as $type){
								$types1[$type->id] = $type->description;
							}
							echo form_dropdown('type', $types1, (isset($_POST['type']) ? $_POST['type'] : ($location ? $location->type_id : '')), 'class="form-control select" id="type" style="width:100%"')
							?>
						</div>

						<div class="form-group code">
	                        <?= lang("code", "code"); ?>
	                        <?php echo form_input('code', $location_code, 'class="form-control tip" id="code" data-bv-notempty="true"'); ?>
	                    </div>
			            <div class="form-group">
			                <?= lang('name', 'name'); ?>
			                <?= form_input('name', set_value('name', ($location ? $location->name : '')), 'class="form-control" id="name" required="required"'); ?>
			            </div>
								<div class="form-group">
									...
									<div class="no-province">
										<?php
											...
											echo form_dropdown('province', $opt_provinces, (isset($_POST['province']) ? $_POST['province'] : ''), 'id="brprovince" class="form-control input-tip select" style="width:100%;"');
										?>
									</div>
								</div>
        ...
        <div class="modal-footer">
            <?php echo form_submit('add_locations', lang('add_locations'), 'class="btn btn-primary"'); ?>
        </div>
    </div>
    <?php echo form_close(); ?>
</div>
...
<script type="text/javascript">
    $(document).ready(function () {
		...
		function biller(){
			...
			<?php
				$multi_project = '';
				if($location && $location->project && $location->project != "null"){
					$projects = json_decode($location->project);
	...
	$(function(){
		...
		function country() {
			$.ajax({
				...
				type : "GET",
				dataType : "JSON",
				data : { id : "<?= ($location?$location->country_id:0) ?>"},
				success:function(data){
		...
		function province(country_id) {
			...
			$.ajax({
				...
				type : "GET",
				data : { country_id : country_id, id : "<?= ($location?$location->province_id:0) ?>"},
				dataType : "JSON",
				success:function(data){
		...
		function district(province_id) {
			...
			$.ajax({
				...
				type : "GET",
				data : { province_id : province_id, id : "<?= ($location?$location->district_id:0) ?>"},
				dataType : "JSON",
				success:function(data){
		...
		function commune(district_id) {
			...
			$.ajax({
				...
				type : "GET",
				data : { district_id : district_id, id : "<?= ($location?$location->commune_id:0) ?>"},
				dataType : "JSON",
				success:function(data){
		...
		function village(commune_id) {
			...
			$.ajax({
				...
				type : "GET",
				data : { commune_id : commune_id, id : "<?= ($location?$location->village_id:0) ?>"},
				dataType : "JSON",
				success:function(data){
					if(data){
