    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            <div class="form-group">
                ...
                <?= form_input('code', set_value('code', ($category ? $category->code : '')), 'class="form-control" id="code" required="required"'); ?>
            </div>

            <div class="form-group">
                <?= lang('category_name', 'name'); ?>
                <?= form_input('name', set_value('name', ($category ? $category->name : '')), 'class="form-control" id="name" required="required"'); ?>
            </div>
			...
			<?php if(!$this->config->item('one_biller')){ ?>
				<div class="form-group">
					...
					echo form_dropdown('biller[]', $bl, (isset($_POST['biller']) ? $_POST['biller'] : ($category ? json_decode($category->biller) : '')), 'class="form-control biller" id="biller" multiple data-placeholder="' . $this->lang->line("select") . " " . $this->lang->line("biller") . '"');
					?>
				</div>
			<?php } if($Settings->project == 1){ ?>	
				<div class="form-group project">
					...
					<div class="no-project-multi">
						...
						echo form_dropdown('project_multi[]', $mpj, (isset($_POST['project_multi']) ? $_POST['project_multi'] : ($category ? json_decode($category->project) : '')), 'id="project_multi" class="form-control input-tip select" data-placeholder="' . lang("select") . ' ' . lang("project") . '"  style="width:100%;" multiple');
						?>
					</div>	
				</div>
			<?php } ?>
			...
			<?php if(POS || $Settings->installment){ ?>
				<div class="form-group">
					...
					echo form_dropdown('warehouse', $wh, (isset($_POST['warehouse']) ? $_POST['warehouse'] : ($category ? $category->warehouse_id : '')), 'class="form-control select" id="warehouse" style="width:100%"')
					?>
				</div>
			<?php } ?>
			...
			<?php if($Settings->installment){ ?>
				...
				<div class="form-group">
					...
					echo form_dropdown('installment', $ln, (isset($_POST['installment']) ? $_POST['installment'] : ($category ? $category->installment : '')), 'class="form-control select" id="installment" style="width:100%"')
					?>
				</div>
			...
			<?php if($pos_settings->table_enable){ ?>
				...
				<div class="form-group">
					...
					echo form_dropdown('type', $types1, (isset($_POST['type']) ? $_POST['type'] : ($category ? $category->type_id : '')), 'class="form-control select" id="type" style="width:100%"')
					?>
				</div>
            ...
            <div class="form-group">
                ...
                echo form_dropdown('parent', $cat, (isset($_POST['parent']) ? $_POST['parent'] : ($category ? $category->parent_id : '')), 'class="form-control select" id="parent" style="width:100%"')
                ?>
            </div>
