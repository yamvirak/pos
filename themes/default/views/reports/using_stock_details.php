...
if ($this->input->post('using_by')) {
    $v .= "&using_by=" . $this->input->post('using_by');
}
...
<script>
    $(document).ready(function () {
        oTable = $('#usingData').dataTable({
            ...
            'sAjaxSource': '<?= site_url('reports/getUsingStockDetailReport/?v=1' . $v); ?>',
			...
			{column_number: 6, filter_default_label: "[<?=lang('using_quantity')?>]", filter_type: "text", data: []},
			{column_number: 7, filter_default_label: "[<?=lang('return_quantity')?>]", filter_type: "text", data: []},
...
<div class="box">
    <div class="box-header">
        ...
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
                    <h2 class="blue"><i class="fa-fw fa fa-filter icon"></i><?= lang('using_stock_details_report'); ?><?php
                        if ($this->input->post('start_date')) {
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    ...
                    <?php echo form_open("reports/using_stock_details"); ?>
                    <div class="row">
						<div class="col-sm-4">
						...
						<div class="col-sm-4">
                            <div class="form-group">
                                <label class="control-label" for="user"><?= lang("using_by"); ?></label>
                                <?php
                                $us_b[""] = lang('select').' '.lang('user');
                                foreach ($users as $user) {
                                    $us_b[$user->id] = $user->last_name . " " . $user->first_name;
                                }
                                echo form_dropdown('using_by', $us_b, (isset($_POST['using_by']) ? $_POST['using_by'] : ""), 'class="form-control" id="user" data-placeholder="' . $this->lang->line("select") . " " . $this->lang->line("using_by") . '"');
                                ?>
                            </div>
                        </div>
							...
							<td class="text_center" style="width:100%">
								...
								<?php 
									...
									if($sale_type_id == 1){
										...
										echo '<div class="bold" style="font-size:15px;font-family: Khmer OS Muol Light;">'.lang('using_stock_details_report_kh').'</div>';
										echo '<div class="bold">'.lang('using_stock_details_report_en').'</div><br>';
								}
								?>
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#pdf').click(function (event) {
            ...
            window.location.href = "<?=site_url('reports/getUsingStockDetailReport/pdf/?v=1'.$v)?>";
            return false;
        });
        $('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=site_url('reports/getUsingStockDetailReport/0/xls/?v=1'.$v)?>";
            return false;
        });
