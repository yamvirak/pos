...
<script>
    $(document).ready(function () {
        oTable = $('#RentalDetailData').dataTable({
			"aaSorting": [[10, "asc"],[0, "desc"]],
			"aLengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "<?= lang('all') ?>"]],
			"iDisplayLength": <?= $Settings->rows_per_page ?>,
			'bProcessing': true, 'bServerSide': true,
			'sAjaxSource': '<?= site_url('reports/getRentalDetailsReport/?v=1'.$v); ?>',
			...
			{column_number: 5, filter_default_label: "[<?=lang('period');?>]", filter_type: "text", data: []},
            {column_number: 6, filter_default_label: "[<?=lang('checked_in_date');?>]", filter_type: "text", data: []},
            {column_number: 7, filter_default_label: "[<?=lang('checked_in_date');?>]", filter_type: "text", data: []},
			{column_number: 8, filter_default_label: "[<?=lang('product');?>]", filter_type: "text", data: []},
			{column_number: 9, filter_default_label: "[<?=lang('unit_price');?>]", filter_type: "text", data: []},
...
<div class="box">
    <div class="box-header">
        <!-- <h2 class="blue"><i class="fa-fw fa fa-dollar"></i><?= lang('rental_details_report'); ?> <?php
            if ($this->input->post('start_date')) {
        ...
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
                    <h2 class="blue"><i class="icon fa fa-dollar tip"></i><?= lang('rental_details_report'); ?></h2>
                </li>
    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    <?php echo form_open("reports/rental_details"); ?>
                    <div class="row">
                ...
                <table style="margin-top: 5px; width:100%;" class="hidden">
                    <th>
                        <tr> 
                                ...
                                <div class="bold" style="font-size:15px;font-family: Khmer OS Muol Light;">
                                        <?= lang('rental_details_report_kh')?>
                                </div> 
                                <div class="bold">
                                        <?= lang('rental_details_report')?>
                                </div><br>
                            </td> 
                        </tr>
                </table>
							...
							<td class="text_center" style="width:100%">
								...
								<?php 
									...
									if($sale_type_id == 1){
										...
										echo '<div class="bold" style="font-size:15px;font-family: Khmer OS Muol Light;">'.lang('rental_details_report_kh').'</div>';
										echo '<div class="bold">'.lang('rental_details_report_en').'</div><br>';
								}
								?>
                ...
                <div class="table-responsive">
                    <table id="RentalDetailData" cellpadding="0" cellspacing="0" border="0"
                        ...
                        <thead>
                         <tr class="primary">
                                ...
                                <th style="width:150px;"><?= lang("checked_out_date"); ?></th>
								<th style="width:150px;"><?= lang("product"); ?></th>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#pdf').click(function (event) {
            ...
            window.location.href = "<?=site_url('reports/getRentalDetailsReport/pdf/?v=1'.$v)?>";
            return false;
        });
		
        $('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=site_url('reports/getRentalDetailsReport/0/xls/?v=1'.$v)?>";
            return false;
        });
