...
<script>
    $(document).ready(function () {
		...
		function transalteType(x){
			if(x == 'addition'){
				return '<?= lang('addition') ?>';
			}else{
				return '<?= lang('subtraction') ?>';
			}
		}
		
        oTable = $('#adjustmentDetailData').dataTable({
            "aaSorting": [[0, "desc"]],
            "aLengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "<?= lang('all') ?>"]],
            "iDisplayLength": <?= $Settings->rows_per_page ?>,
            'bProcessing': true, 'bServerSide': true,
            'sAjaxSource': '<?= site_url('reports/getAdjustmentDetailsReport/?v=1' . $v); ?>',
			...
			"fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                ...
                for (var i = 0; i < aaData.length; i++) {
                    grand_cost += parseFloat(aaData[aiDisplay[i]][7]);
                }
                var nCells = nRow.getElementsByTagName('th');
                nCells[7].innerHTML = currencyFormat(parseFloat(grand_cost));
...
<div class="box">
    ...
    <div class="sub_menu"></div>
        ...
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
                    <h2 class="blue"><i class="fa-fw fa fa-filter"></i><?= lang('adjustment_details_report'); ?><?php
                        if ($this->input->post('start_date')) {
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    ...
                    <?php echo form_open("reports/adjustment_details"); ?>
                    <div class="row">
						<div class="col-sm-4">
                            ...
                            <?php 
                                ...
                                <?php 
                                    ...
                                    if($sale_type_id == 1){
                                        ...
                                        echo '<div class="bold" style="font-size:15px;font-family: Khmer OS Muol Light;">'.lang('adjustment_details_report_kh').'</div>';
                                        echo '<div class="bold">'.lang('adjustment_details_report_en').'</div><br>';
                                }
                ...
                <div class="table-responsive">
                    <table id="adjustmentDetailData" class="table table-bordered table-condensed table-hover table-striped">
                        <thead>
							<tr>
								...
								<th class="col-xs-1"><?= lang("total_cost")  ?></th>
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#pdf').click(function (event) {
            ...
            window.location.href = "<?=site_url('reports/getAdjustmentDetailsReport/pdf/?v=1'.$v)?>";
            return false;
        });
        $('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=site_url('reports/getAdjustmentDetailsReport/0/xls/?v=1'.$v)?>";
            return false;
        });
...
<style type="text/css">
	<?php if(!$Owner && !$Admin && !$this->session->userdata('show_cost')){ ?>
		#adjustmentDetailData td:nth-child(8),#adjustmentDetailData th:nth-child(8) {
