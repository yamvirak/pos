			...
			<table>
						...
						<th>
							<table>
								<tr>
									...
									<td class="text_center" style="width:40%"><span style="font-size:<?= $font_size+5 ?>px"><b><i><?= lang('SUPPLIER_BALANCE_STATEMENT') ?></i></b></span></td>
				...
				<tbody>
					<?php
						$tbody = '';
						$tfooter = '';
						$grand_total = 0; $paid = 0;  $total_balance = 0; $total_return = 0;	$i=1;
						if($rows){
							foreach ($rows as $row){
								$balance = $row->balance;
						...
						if($Settings->payment_expense==1 && count($row_expenses) > 0){
							foreach ($row_expenses as $row_expense){
								$balance = $row_expense->balance;
								if($balance !=0){
									$grand_total += $row_expense->grand_total;
									$paid += $row_expense->paid;
									$total_balance += $balance;
									$total_return += 0;
									$tbody .='<tr>
												<td class="text_center">'.$i.'</td>
												<td class="text_center">'.$this->cus->hrld($row_expense->date).'</td>
												<td class="text_center">'.$row_expense->reference.'</td>
												<td class="text_right">'.$this->cus->formatMoney($row_expense->grand_total).'</td>
												<td class="text_right">'.$this->cus->formatMoney(0).'</td>
												<td class="text_right">'.$this->cus->formatMoney($row_expense->paid).'</td>
												<td class="text_right">'.$this->cus->formatMoney($balance).'</td>
											</tr>';
									$i++;		
								}	
