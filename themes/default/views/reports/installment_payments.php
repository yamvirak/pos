...
<script>
    $(document).ready(function () {
		oTable = $('#InstallmentPaymentTable').dataTable({
            "aaSorting": [[1, "desc"]],
            "aLengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "<?=lang('all')?>"]],
            "iDisplayLength": <?=$Settings->rows_per_page?>,
            'bProcessing': true, 'bServerSide': true,
            'sAjaxSource': '<?=site_url('reports/getInstallmentPaymentsReport?v=1&'. $v)?>',
            ...
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
				var principal_paid = 0, interest_paid = 0, payment_paid = 0, penalty_paid = 0;
				for (var i = 0; i < aaData.length; i++) {
                    principal_paid += parseFloat(aaData[aiDisplay[i]][7]);
                ...
                nCells[7].innerHTML = currencyFormat(parseFloat(principal_paid));
...
<?php echo form_open("reports/installment_payments", ' id="form-submit" '); ?>
 
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-calendar"></i><?= lang('installment_payments'); ?></h2>
        <div class="box-icon">
            <ul class="btn-tasks">
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="table-responsive">
                    <table id="InstallmentPaymentTable" class="table table-bordered table-hover table-striped table-condensed">
                        <thead>
							<tr>
								<th><?= lang("date"); ?></th>
								...
								<th><?= lang("payment") ?><br/><small>( <?= lang('paid') ?> )</small></th>
								<th><?= lang("penalty") ?><br/><small>( <?= lang('paid') ?> )</small></th>
								<th><?= lang("type"); ?></th>
							</tr>
                        </thead>
                        <tbody>
...
<script type="text/javascript">
    $(document).ready(function () {
		...
		$('#pdf').click(function (event) {
            ...
            window.location.href = "<?=site_url('reports/getInstallmentPaymentsReport/pdf/0?v=1'.$v)?>";
            return false;
        });
        $('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=site_url('reports/getInstallmentPaymentsReport/0/xls/?v=1'.$v)?>";
            return false;
        });
